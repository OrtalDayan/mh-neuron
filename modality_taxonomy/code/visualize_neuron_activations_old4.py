...
def main():
    ...
    if args.fig3:
        ...
        # Tracks which rank_idx has already been assigned per (layer, nidx).
        # Prevents two panels sharing the same neuron (e.g. (e) and (f),
        # both layer 21 / neuron 6100) from falling back to the same rank.
        used_rank_by_neuron = {}                                        # (layer, nidx) -> set of int rank_idx
        ...
        for entry in fig3_neurons:
            ...
            # ── Load Top-N Heap data (full mode) or fall back to direct
            #    image + rank-0 activation lookup (test / patch-only mode) ──────
            topn_available = True                                       # assume heap data exists
            try:
                top_n_sids, top_n_acts, global_max = load_topn_heap_layer(
                    topn_heap_dir, layer)
            except FileNotFoundError:
                # Test / patch-only mode: Top-N Heap was never built.
                # patch_fig3_activations.py writes each fig3 image's
                # activations directly to rank slot 0 of act_pattern_raw,
                # so we can bypass the heap entirely.
                topn_available = False
                print(f'  [test mode] Top-N Heap missing for layer {layer} — ')
                print(f'  using direct image lookup + act_pattern rank 0.')
            ...
            try:
                ...
                print(f'  Run patch_fig3_activations.py first (step 6 does this')
                print(f'  automatically before calling the viz script).')
                continue

            if topn_available:
                # ── Full mode: find the rank that holds the target image ──────
                ...
                if rank_idx is not None:
                    ...
                    # Image not in this neuron's top-N — fall back to the
                    # highest unused rank so panels sharing a neuron show
                    # different images (handles the (e)/(f) same-neuron case).
                    neuron_key = (layer, nidx)
                    already_used = used_rank_by_neuron.get(neuron_key, set())
                    sorted_ranks = np.argsort(-top_n_acts[nidx])
                    rank_idx = None
                    for candidate in sorted_ranks:
                    ...
                    print(f'  top-N. Falling back to rank {rank_idx} ')
                    print(f'  (first unused rank).')

                # Record rank as used for this neuron
                neuron_key = (layer, nidx)
                used_rank_by_neuron.setdefault(neuron_key, set()).add(rank_idx)
                ...
                # ── Test / patch-only mode: use target image directly ─────────
                # patch_fig3_activations.py always writes to rank slot 0, so
                # we read vis_acts / txt_acts from rank 0 of act_pattern_raw.
                rank_idx = 0                                            # patch always writes to rank 0
                img_id = target_coco_id                                 # use the known fig3 image ID

                # Try id_to_fn lookup first; fall back to direct {id}.jpg path
                filename = id_to_fn.get(img_id) or id_to_fn.get(
                    str(int(img_id)))                                   # handle unpadded IDs in detail_23k
                if filename:
                    img_path = os.path.join(args.coco_img_dir, filename)
                else:
                    img_path = os.path.join(                            # direct fallback: coco_id.jpg
                        args.coco_img_dir, f'{img_id}.jpg')
                if not os.path.exists(img_path):
                    print(f'  ERROR: Image not found: {img_path}, skipping')
                    continue
                print(f'  [test mode] Using direct image: {img_path}')

            # Load image (shared path for both modes)
            img = Image.open(img_path).convert('RGB')
            img_array = np.array(img)

            # Extract activations (shared between FT and PMBT)
            # rank_idx = 0 in test mode (patch writes to slot 0),
            # or the located rank in full mode.
            ...
            # Get text tokens — look up by img_id (works for both modes)
            # In test mode img_id == target_coco_id so descriptions lookup
            # uses the same key as the description file.
            desc_key = img_id                                           # primary lookup key
            if desc_key not in descriptions:                            # try unpadded fallback
                desc_key = str(int(img_id))
            if desc_key in descriptions:
                tokens = get_description_tokens(
                    tokenizer, desc_key, descriptions, args.model_type)
                if len(tokens) != txt_len and txt_len > 0:
                    print(f'  NOTE: token count mismatch: ')
                    print(f'  tokenizer={len(tokens)}, saved={txt_len}')
            else:
                tokens = [f'tok_{i}' for i in range(max(txt_len, 1))]
                print(f'  WARNING: No description for {img_id}, ')
                print(f'  using placeholders')
            ...
            if has_pmbt:
                try:
                    # Try permutation-specific filename first, then default
                    try:
                    ...
                    pmbt_pv = pmbt_info.get('pv', pmbt_info.get('p_visual', 0.0))
                    pmbt_pt = pmbt_info.get('pt', pmbt_info.get('p_text', 0.0))
                    pmbt_pm = pmbt_info.get('pm', pmbt_info.get('p_multimodal', 0.0))
                    pmbt_pu = pmbt_info.get('pu', pmbt_info.get('p_unknown', 0.0))
        ...
        def _make_combined_grid(paths, entries, suffix, title_extra=''):
            ...
            fig_c.suptitle(f'Figure 3 — {args.model_name}',
