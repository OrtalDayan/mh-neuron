...
def create_neuron_panel(img_array, vis_acts, txt_acts, txt_len,
                        ...
                        patch_grid=24, model_name='', pmbt_label=None):
    ...
    # Short label map used for both FT and PMBT classification display
    _label_map = {                                                  # concise labels for title line 1
        'visual':     'Visual',
        'text':       'Text',
        'multimodal': 'Multi-modal',
        'unknown':    'Unknown',
    }
    ft_label_display = _label_map.get(label, label.capitalize())    # FT classification (short)

    # PMBT classification: shown only when pmbt_label is provided
    if pmbt_label is not None:
        pmbt_display = _label_map.get(                              # PMBT classification (short)
            pmbt_label, pmbt_label.capitalize())
        pmbt_part = f' | PMBT: {pmbt_display}'                      # appended to line 1
    else:
        pmbt_part = ''                                              # omit PMBT section if unavailable
    ...
    ax_header.axis('off')
    # Line 1: FT classification | PMBT classification | Layer | Neuron
    # Line 2: Max visual activation value | Max text activation value
    header_text = (
        f'FT: {ft_label_display}{pmbt_part}'
        f' | Layer: {layer} | Neuron: {neuron_idx}\n'
        f'Max visual act: {max_vis_act} | Max text act: {max_txt_act}'
    )
...
def main():
    ...
    if args.fig3:
        ...
        for entry in fig3_neurons:
            ...
            # ── Pre-fetch PMBT label for combined title (line 1) ──────
            # Load PMBT classification label before creating the FT panel
            # so both FT and PMBT types can appear together in the title.
            pmbt_label_val = None                                       # default: no PMBT info
            if has_pmbt:
                try:
                    try:                                                # prefer permutation file
                        _pmbt_pre = load_neuron_labels(
                            args.pmbt_data_dir, layer, args.model_type,
                            label_file='neuron_labels_permutation.json')
                    except FileNotFoundError:                           # fall back to default file
                        _pmbt_pre = load_neuron_labels(
                            args.pmbt_data_dir, layer, args.model_type,
                            label_file='neuron_labels.json')
                    pmbt_label_val = _pmbt_pre[nidx].get('label', label)  # PMBT type string
                except (FileNotFoundError, IndexError, KeyError, TypeError):
                    pmbt_label_val = None                               # silently ignore; logged below
            ...
            fig_ft = create_neuron_panel(
                ...
                pmbt_label=pmbt_label_val,                          # show PMBT type in title line 1
            )
