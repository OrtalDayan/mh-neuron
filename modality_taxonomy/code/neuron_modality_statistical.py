...
Extends Xu et al. (MM 2025) Phases 1-2 with principled statistical testing.
  ...
  Permutation-test — (NEW) For each neuron:
      ...
      4. Compute p-values, classify at α=0.001
...
Scope: LLM FFN neurons only — output of act_fn = SiLU(gate_proj(x)),
       ...
       With --hook_point gate_up: hooks down_proj input = SiLU(gate_proj(x)) * up_proj(x),
       the full SwiGLU intermediate (Geva's conceptual equivalent).

       With --hook_point attn: hooks self_attn.o_proj input = softmax(QK^T/√d)V,
       reduced to per-head L2 norms.  Classifies attention heads (n_heads per
       layer, e.g. 32) instead of FFN neurons.  This is the attention analogue
       of gate_up for FFN — the actual content entering the residual stream.
...
def get_layer_names(model_type='llava-hf', n_layers=32, model=None):
    ...
    Args:
        model_type: model backend name
        n_layers: number of LLM layers
        model: optional — if provided, auto-detects the correct layer prefix
               for llava-ov (transformers v4.48 vs v4.52 structure difference)
    """
    if model_type == 'llava-hf':
        ...
        #   → .language_model (Qwen2_5_VLTextModel) → .layers[i] → .mlp (Qwen2MLP) → .act_fn (SiLU)
        if model is not None and not hasattr(model.model, 'language_model'):
            prefix = 'model.layers'                                    # older transformers without language_model
        else:
            prefix = 'model.language_model.layers'                     # current transformers (≥ 4.49)
        suffix = 'mlp.act_fn'
    elif model_type == 'llava-ov':
        # LlavaOnevisionForConditionalGeneration._modules tree:
        #   ['model'] → LlavaOnevisionModel
        #     ['language_model'] → Qwen2Model
        #       ['layers'] → ModuleList → [i] → .mlp.act_fn
        prefix = 'model.language_model.layers'
        suffix = 'mlp.act_fn'
    elif model_type == 'idefics2':
        # Idefics2ForConditionalGeneration → .model (Idefics2Model)
        #   → .text_model (MistralModel) → .layers[i] → .mlp.act_fn (SiLU)
...
def get_down_proj_names(model_type='llava-hf', n_layers=32, model=None):
    """Return down_proj layer names — we hook with retain_input=True to capture
    SiLU(gate_proj(x)) * up_proj(x), the FULL intermediate activation.

    This is the actual value that gets projected by down_proj into the residual
    stream.  Unlike get_layer_names() which returns act_fn (gate-only),
    this captures the element-wise product of both SwiGLU branches.

    Usage: TraceDict(model, names, retain_input=True)
           then td[name].input[0]  → (batch, seq, intermediate_size)
    """
    if model_type == 'llava-hf':
        prefix = 'model.language_model.layers'
        ...
        prefix = 'language_model.model.layers'
        suffix = 'feed_forward.w2'              # InternLM2 uses w1/w2/w3 naming; w2 = down_proj
    elif model_type == 'qwen2vl':
        if model is not None and not hasattr(model.model, 'language_model'):
            prefix = 'model.layers'
        else:
        ...
        prefix = 'model.language_model.layers'
        suffix = 'mlp.down_proj'
    elif model_type in ('qwen25vl-7b', 'qwen25vl-3b'):
        prefix = 'model.language_model.layers'
        suffix = 'mlp.down_proj'
    elif model_type == 'idefics2':
        prefix = 'model.text_model.layers'
...
def get_o_proj_names(model_type='llava-hf', n_layers=32, model=None):
    """Return o_proj layer names — hook with retain_input=True to capture
    the full attention output BEFORE the output projection.

    This is the attention analogue of gate_up for FFN: the actual content
    that enters the residual stream via o_proj.

    The attention computation produces:
        attn_output = softmax(Q @ K.T / sqrt(d)) @ V   ← we capture THIS
        output = o_proj(attn_output)                     ← projects to residual stream

    Shape: (batch, seq, hidden_size) which reshapes to (seq, n_heads, head_dim).
    We reduce to per-head L2 norms → (seq, n_heads) for PMBT classification.

    Usage: TraceDict(model, names, retain_input=True)
           then td[name].input[0]  → (batch, seq, hidden_size)
    """
    if model_type == 'llava-hf':
        prefix = 'model.language_model.layers'
        ...
        prefix = 'language_model.model.layers'
        suffix = 'attention.wo'              # InternLM2 uses wqkv/wo naming; wo = o_proj
    elif model_type == 'qwen2vl':
        if model is not None and not hasattr(model.model, 'language_model'):
            prefix = 'model.layers'
        else:
...
def _get_num_attention_heads(model, model_type):
    """Extract the number of attention heads from the model config.

    Returns:
        int — number of attention heads (e.g. 32 for LLaMA-2/3/InternLM2,
              28 for Qwen2-7B)
    """
    for cfg_attr in [
        'config.text_config.num_attention_heads',    # llava-hf, qwen2vl, idefics2
        'language_model.config.num_attention_heads',  # internvl
        'config.num_attention_heads',                 # original llava, fallback
    ]:
        obj = model
        try:
            for part in cfg_attr.split('.'):
            ...
            continue
    raise RuntimeError(
        f'Cannot determine num_attention_heads for model_type={model_type}. '
        f'Check model config structure.')


def _postprocess_acts(raw_acts, hook_point, n_heads=None):
    """Post-process raw activation tensor based on hook point.

    For 'gate':    passthrough — SiLU outputs, already scalar per neuron.
    For 'gate_up': passthrough — gate*up products, scalar per neuron.
    For 'attn':    reshape (seq, hidden) → (seq, n_heads, head_dim),
                   compute per-head L2 norm → (seq, n_heads).
                   Each scalar = how much head h contributes at this position.

    Args:
        raw_acts: (seq_len, dim) tensor from TraceDict
        hook_point: 'gate' | 'gate_up' | 'attn'
        n_heads: required when hook_point='attn'

    Returns:
        (seq_len, n_units) tensor — one scalar per unit per token position.
    """
    if hook_point == 'attn':
        assert n_heads is not None, 'n_heads required for attn hook_point'
        seq_len, hidden_size = raw_acts.shape
        head_dim = hidden_size // n_heads
        # (seq_len, hidden_size) → (seq_len, n_heads, head_dim)
        reshaped = raw_acts.reshape(seq_len, n_heads, head_dim)
        # Per-head L2 norm → (seq_len, n_heads), non-negative
        return torch.norm(reshaped.float(), dim=-1)
    else:
        return raw_acts


def _precompute_importance_weights(model, model_type, hook_point, layers,
                                   n_heads=None):
    """Precompute per-unit output-projection L2 norms for importance weighting.

    For gate/gate_up: L2 norm of each down_proj column (hidden_dim,) → scalar.
        Measures how strongly neuron n's activation affects the residual stream.
    For attn: Frobenius norm of each head's (hidden_dim, head_dim) block in o_proj.
        Measures how strongly head h's output affects the residual stream.

    Returns dict: layer_index → 1-D numpy array of norms, shape (n_units,).
    """
    n_layers = max(layers) + 1

    with torch.no_grad():
        if hook_point in ('gate', 'gate_up'):
            proj_names = get_down_proj_names(model_type, n_layers, model=model)
            weights = {}
            for l in layers:
                mod = model
                for p in proj_names[l].split('.'):
                    mod = getattr(mod, p)
                # mod.weight: (hidden_dim, intermediate_dim) — column n = neuron n
                norms = torch.norm(mod.weight.float(), dim=0).cpu().numpy()
                weights[l] = norms
            return weights

        elif hook_point == 'attn':
            assert n_heads is not None
            proj_names = get_o_proj_names(model_type, n_layers, model=model)
            weights = {}
            for l in layers:
                mod = model
                for p in proj_names[l].split('.'):
                    mod = getattr(mod, p)
                # mod.weight: (hidden_dim, hidden_dim) → reshape to (hidden_dim, n_heads, head_dim)
                w = mod.weight.float()
                head_dim = w.shape[0] // n_heads
                w_heads = w.reshape(w.shape[0], n_heads, head_dim)
                # Frobenius norm per head → (n_heads,)
...
def build_prompt(img_id, text_source, model_type='llava-hf',
    ...
    if model_type == 'llava-hf':
        ...
        # Idefics2: prompt is built inside prepare_inputs_idefics2()
        # using processor.apply_chat_template(). Return description text
        # for reference / token counting.
        return text
    else:
...
def count_description_tokens(img_id, model_type, processor, descriptions):
    ...
    if model_type == 'llava-hf':
        ...
        # Strip any tokens that match <IMG_CONTEXT> — must stay consistent
        # with the filtering in prepare_inputs_internvl (step 4a).
        img_ctx_id = tokenizer.convert_tokens_to_ids('<IMG_CONTEXT>')
        desc_ids = [t for t in desc_ids if t != img_ctx_id]
        return len(desc_ids)
    elif model_type == 'llava-ov':
        ...
        # For Idefics2, processor is AutoProcessor — access .tokenizer
        # Tokenise the description text standalone for counting.
        tokenizer = processor.tokenizer
        desc_ids = tokenizer.encode(desc_text, add_special_tokens=False)
        return len(desc_ids)
...
def classify_neuron_permutation(vis_acts, txt_acts, n_permutations=1000,
                                 alpha=0.001, min_high_tokens=5, rng=None):
...
def classify_neuron_permutation_gpu(vis_acts, txt_acts, n_permutations=1000,
                                    alpha=0.001, min_high_tokens=5, device='cuda',
                                    seed=42):
...
def load_model_hf(hf_id, device):
    ...
    # Detect LLaVA-Next vs LLaVA-1.5 from config
...
def load_model_internvl(model_path, device):
    ...
    # Ensure the model knows which token ID to look for in forward().
    # InternVL's modeling_internvl_chat.py uses self.img_context_token_id
    # to find visual token positions for embedding replacement.  If this
    # attribute is None (e.g. missing from config), the comparison
    # input_ids == None yields a scalar False instead of a boolean mask,
    # causing "AttributeError: 'bool' object has no attribute 'sum'".
    if getattr(model, 'img_context_token_id', None) is None:
        model.img_context_token_id = image_token_id
        print(f'  Set model.img_context_token_id = {image_token_id}')
    elif model.img_context_token_id != image_token_id:
        print(f'  WARNING: model.img_context_token_id={model.img_context_token_id} '
              f'!= tokenizer IMG_CONTEXT={image_token_id}, using model value')
...
def prepare_inputs_internvl(tokenizer, model, img, desc_text, device,
    ...
    # 4a. Strip any stray IMG_CONTEXT tokens from pre/post IDs.
    #     The tokenizer may map <img>, </img>, or description subwords to
    #     the same ID as <IMG_CONTEXT>.  The model's forward() selects ALL
    #     positions matching img_context_token_id, so extras cause a size
    #     mismatch between input_embeds[selected] and vit_embeds.
    n_stray_pre  = sum(1 for t in pre_ids  if t == image_token_id)        # count before filtering
    n_stray_post = sum(1 for t in post_ids if t == image_token_id)        # count before filtering
    pre_ids  = [t for t in pre_ids  if t != image_token_id]               # remove accidental IMG_CONTEXT
    post_ids = [t for t in post_ids if t != image_token_id]               # remove accidental IMG_CONTEXT
    if n_stray_pre + n_stray_post > 0:
        print(f'    [internvl] Stripped {n_stray_pre + n_stray_post} stray '
              f'IMG_CONTEXT tokens (pre={n_stray_pre}, post={n_stray_post})')
...
def prepare_inputs_llava_next(processor, img, desc_text, device,
                              image_token_id, text_source='generated'):
    """Prepare teacher-forcing inputs for LLaVA-Next (e.g. LLaMA3 backbone).

    Uses processor.apply_chat_template() to build the correct prompt format
    for the model's native chat template (LLaMA-3, Mistral, etc.).
...
def load_model_qwen2vl(model_path, device):
    """Load Qwen2-VL or Qwen2.5-VL from a local directory.

    Auto-detects the model generation from config.model_type:
    ...
    _ensure_act_fn_is_module(model, 'qwen2vl')
...
def load_model_idefics2(model_path, device):
    """Load Idefics2-8B from a local directory or HF Hub.

    Uses AutoModelForVision2Seq which auto-resolves to
    Idefics2ForConditionalGeneration. The model uses a Mistral-7B
    text backbone with a SigLIP vision encoder and perceiver resampler.

    Returns: (model, processor, image_token_id)
        - model          : Idefics2ForConditionalGeneration
        - processor      : AutoProcessor (text tokenizer + image processor)
        - image_token_id : int — token ID for <image> placeholder
    """
    from transformers import AutoProcessor, AutoModelForVision2Seq           # standard HF auto classes

    processor = AutoProcessor.from_pretrained(model_path)                    # loads tokenizer + image processor
    model = AutoModelForVision2Seq.from_pretrained(                          # load Idefics2 model
        model_path,
        torch_dtype=torch.float16,                                           # float16 — Idefics2 standard dtype
        low_cpu_mem_usage=True,                                              # reduce peak RAM during loading
    ).to(device).eval()                                                      # move to GPU, disable dropout

    # Idefics2 uses <image> as the visual placeholder token
    image_token_id = processor.tokenizer.convert_tokens_to_ids('<image>')

    return model, processor, image_token_id


def prepare_inputs_idefics2(processor, img, desc_text, device,
                            image_token_id, text_source='generated'):
    """Prepare teacher-forcing inputs for Idefics2.

    Constructs the Idefics2 chat template with the generated description
    as the assistant response (teacher forcing). The processor handles
    image preprocessing and token expansion internally.
    ...
    if text_source == 'generated':
        ...
        # COCO caption mode: caption as user text
        messages = [
            {"role": "user", "content": [
                {"type": "image"},
                {"type": "text", "text": desc_text},
    ...
    # 2. Format messages → text string with special tokens
    prompt_text = processor.apply_chat_template(                               # converts messages to formatted string
        messages, add_generation_prompt=False)                                 # teacher forcing — no generation prompt

    # 3. Tokenize text + preprocess image together
    inputs = processor(                                                        # Idefics2Processor handles text + image
        text=prompt_text,
        images=[img],                                                          # pass PIL image in a list
        return_tensors='pt',
    ).to(device)

    # 3. Build visual mask: True where input_ids == image token
    ids_cpu = inputs['input_ids'][0].cpu().numpy()
    visual_mask = (ids_cpu == image_token_id)
    n_vis_tokens = int(visual_mask.sum())
...
def parse_args():
    ...
    p.add_argument('--model_type', default='llava-hf',
                   choices=['llava-hf', 'llava-liuhaotian', 'internvl', 'qwen2vl',
                            ...
                            'llava-ov', 'llava-llama3', 'llava-mistral'],
                   help='"llava-hf" for llava-hf/llava-1.5-7b-hf, '
                        '"llava-liuhaotian" for liuhaotian/llava-v1.5-7b, '
                        '"internvl" for InternVL2.5-8B, '
                        '"qwen2vl" for Qwen2-VL-7B / Qwen2.5-VL, '
                        '"idefics2" for Idefics2-8B, '
                        '"llava-ov" for llava-hf/llava-onevision-qwen2-7b-ov-hf, '
                        '"llava-llama3" for llava-hf/llama3-llava-next-8b-hf, '
                        '"llava-mistral" for llava-hf/llava-v1.6-mistral-7b-hf')
    ...
    p.add_argument('--hook_point', default='gate',
                   ...
                   help='"gate" = SiLU(gate_proj(x)) — gate activation only '
                        '(Xu et al. method, original PMBT). '
                        '"gate_up" = SiLU(gate_proj(x)) * up_proj(x) — full '
                        'intermediate value that actually flows to down_proj '
                        '(Geva conceptual equivalent in SwiGLU). '
                        '"attn" = input to self_attn.o_proj — full attention '
                        'output softmax(QK^T/sqrt(d))V, reduced to per-head '
                        'L2 norms. Classifies attention heads (n_heads per '
                        'layer) instead of FFN neurons (d_ffn per layer).')
    p.add_argument('--importance_weight', action='store_true',
                   help='Weight activations by output-projection L2 norms '
                        'before classification. For gate/gate_up: down_proj '
                        'column norms (neuron contribution magnitude to '
                        'residual stream). For attn: o_proj per-head block '
                        'Frobenius norms. Converts raw activations into '
                        'approximate residual-stream contributions, bridging '
                        'the gap between "fires differently" and "matters '
    ...
    p.add_argument('--output_suffix', default='',
                   help='Extra suffix for output dirs (e.g. _min100_max2048). '
                        'Appended after hook suffix: llm_permutation_gate_up_min100_max2048/')
    p.add_argument('--skip_permutation', action='store_true',
                   help='Skip the PMBT (permutation-test) classification block. '
                        'Only the fixed-threshold (FT) classification is written. '
                        'Used for Xu-reproduction runs where we want only FT-on-gate '
                        'classifications, not PMBT outputs that would clutter the '
...
def main(args=None):
    ...
    # (same hook names, same SwiGLU FFN), but different layer counts.
    # Save original for layer count lookup before normalizing.
    _original_model_type = args.model_type
    if args.model_type in ('qwen25vl-7b', 'qwen25vl-3b'):
        args.model_type = 'qwen2vl'

    # Handle llava-llama3: loaded via load_model_hf (auto-detects LlavaNext),
    # but uses apply_chat_template for prompt building (not USER/ASSISTANT format).
    # We normalize to 'llava-hf' for loading and layer names, then override
    # the input preparation to use chat template style.
    _is_llava_llama3 = False
    if args.model_type in ('llava-llama3', 'llava-mistral'):  # LLaVA-Next siblings: load+hook as llava-hf, chat-template inputs
        _orig_mt = args.model_type
        _is_llava_llama3 = True
        if args.hf_id == 'llava-hf/llava-1.5-7b-hf':  # still the default — override
            _default_hf = ('llava-hf/llama3-llava-next-8b-hf' if _orig_mt == 'llava-llama3'
                           else 'llava-hf/llava-v1.6-mistral-7b-hf')
            args.hf_id = args.model_path or args.original_model_path or _default_hf
        args.model_type = 'llava-hf'  # load and hook like llava-hf
        print(f'  [{_orig_mt}] Load via llava-hf, inputs via chat template, hf_id={args.hf_id}')
    ...
    # ─── Apply --num_images limit ───
    if args.num_images < len(sampled_ids):
        sampled_ids = sampled_ids[:args.num_images]
        print(f'Limited to {len(sampled_ids)} images (--num_images {args.num_images})')
    ...
    if args.model_type == 'llava-hf':
        ...
        _path = args.model_path or args.original_model_path or 'modern_vlms/pretrained/llava-onevision-qwen2-7b-ov-hf'
        ...
        _path = args.model_path or args.original_model_path
        if not _path:
            raise ValueError(
                f"qwen2vl model_type requires --model_path or --original_model_path. "
                f"Refusing to silently fall back to a hardcoded default — that fallback "
                f"would mismatch dimensions for qwen25vl-3b (intermediate_size=11008) "
                f"vs Qwen2-VL-7B / Qwen2.5-VL-7B (intermediate_size=18944) and corrupt "
                f"the labels file. Pass --original_model_path explicitly."
            )
        ...
        _path = args.model_path or args.original_model_path or 'modern_vlms/pretrained/idefics2-8b'
        print(f'Loading Idefics2 model: {_path} …')
        model, processor, image_token_id = load_model_idefics2(_path, device)
    else:
    ...
    _n_layers_map = {
        ...
        'idefics2': 32,
        'qwen25vl-3b': 36,  # Qwen2.5-VL-3B: 36 layers, 11008 intermediate, 16 heads
    }
    # Look up by original model type first (before normalization), fall back to normalized
    n_layers_model = _n_layers_map.get(_original_model_type,
                       _n_layers_map.get(args.model_type, 32))
    if args.layer_end > n_layers_model:
        ...
        n_layers_total = args.layer_end - args.layer_start
    all_layer_names = get_layer_names(args.model_type, n_layers_model, model=model)
    _n_heads = None  # only set for attn hook_point
    if args.hook_point == 'gate_up':
        all_layer_names = get_down_proj_names(args.model_type, n_layers_model, model=model)
        print(f'Hook point: gate_up — hooking down_proj input = SiLU(gate_proj(x)) * up_proj(x)')
    elif args.hook_point == 'attn':
        all_layer_names = get_o_proj_names(args.model_type, n_layers_model, model=model)
        _n_heads = _get_num_attention_heads(model, args.model_type)
        print(f'Hook point: attn — hooking o_proj input = softmax(QK^T/√d)V, '
              f'{_n_heads} heads per layer')
    else:
        print(f'Hook point: gate — hooking act_fn output = SiLU(gate_proj(x))')
    ...
    # ── Pre-create ALL output directories immediately ─────────
    # So they're visible on the filesystem before the long processing loop.
    _hook_suffix = {'gate_up': '_gate_up', 'attn': '_attn'}.get(args.hook_point, '')
    if args.importance_weight:
        _hook_suffix += '_iw'
    _ft_base = os.path.join(args.output_dir, args.model,
                            f'llm_fixed_threshold{_hook_suffix}{args.output_suffix}')
    _pm_base = os.path.join(args.output_dir, args.model,
                            f'llm_permutation{_hook_suffix}{args.output_suffix}')
    os.makedirs(_ft_base, exist_ok=True)
    os.makedirs(_pm_base, exist_ok=True)
    os.makedirs(os.path.join(_ft_base, 'topn_heap'), exist_ok=True)
    os.makedirs(os.path.join(_ft_base, 'act_pattern_raw'), exist_ok=True)
    for ln in my_layer_names:
        os.makedirs(os.path.join(_ft_base, ln), exist_ok=True)
        os.makedirs(os.path.join(_pm_base, ln), exist_ok=True)
    print(f'Output dirs pre-created:')
    print(f'  FT:   {_ft_base}')
    print(f'  PMBT: {_pm_base}')
    ...
    if _is_llava_llama3:
        ...
        probe_inputs, _, _probe_nvis = prepare_inputs_llava_next(
        ...
        _desc = descriptions.get(probe_id, '') if descriptions else ''
        probe_inputs, _, _probe_nvis = prepare_inputs_idefics2(
            processor, probe_img, _desc, device,
            image_token_id, args.text_source)
    else:
    ...
    with torch.no_grad():
        with TraceDict(model, my_layer_names, retain_input=_retain_input) as td:
            model(**probe_inputs)

    if _retain_input:
        inp = td[my_layer_names[0]].input
        out = inp[0] if isinstance(inp, tuple) else inp   # unwrap tuple if needed
    else:
        out = td[my_layer_names[0]].output          # output of act_fn = gate only
    if isinstance(out, tuple):
        out = out[0]
    # For attn: reshape (seq, hidden) → per-head L2 norms (seq, n_heads)
    out = _postprocess_acts(out[0], args.hook_point, _n_heads)
    n_neurons = out.shape[-1]   # FFN: 11008/14336/18944; attn: n_heads (e.g. 32)
    print(f'Units per layer: {n_neurons}'
          f'{f" ({_n_heads} attention heads)" if args.hook_point == "attn" else ""}')

    # ── Precompute importance weights (down_proj / o_proj norms) ──────
    _importance_weights = None
    if args.importance_weight:
        print(f'\nPrecomputing importance weights (output-projection L2 norms) …')
        _importance_weights = _precompute_importance_weights(
            model, args.model_type, args.hook_point, my_layers, n_heads=_n_heads)
        _iw0 = _importance_weights[my_layers[0]]
        print(f'  Layer {my_layers[0]}: norm range [{_iw0.min():.4f}, {_iw0.max():.4f}], '
              f'mean {_iw0.mean():.4f}, shape {_iw0.shape}')
        print(f'  Importance weighting ENABLED for {args.hook_point} hook')
    else:
        print(f'\n  Importance weighting disabled (use --importance_weight to enable)')
    ...
    for sample_idx, img_id in enumerate(tqdm(sampled_ids, desc='Top-N Heap')):
        ...
        if args.model_type == 'internvl':
            ...
            _desc = descriptions.get(img_id, '') if descriptions else \
                    (captions_dict[img_id][0] if captions_dict else '')
            inputs, visual_positions, _ = prepare_inputs_idefics2(
                processor, img, _desc, device,
                image_token_id, args.text_source)
        elif _is_llava_llama3:
            _desc = descriptions.get(img_id, '') if descriptions else \
                    (captions_dict[img_id][0] if captions_dict else '')
            inputs, visual_positions, _ = prepare_inputs_llava_next(
                processor, img, _desc, device,
                image_token_id, args.text_source)
        elif args.model_type == 'llava-hf':
        ...
        if args.text_source == 'generated':
            _cnt_model_type = 'llava-ov' if _is_llava_llama3 else args.model_type
            desc_count = count_description_tokens(
                img_id, _cnt_model_type, processor, descriptions)
        ...
        for li, layer_name in enumerate(my_layer_names):
            # Extract activations based on hook point
            if _retain_input:
                inp = td[layer_name].input
                out = inp[0] if isinstance(inp, tuple) else inp
            else:
                out = td[layer_name].output          # gate only (act_fn output)
            if isinstance(out, tuple):
                out = out[0]
            # out: (1, seq_len, dim); for attn → per-head norms (seq_len, n_heads)
            acts = _postprocess_acts(out[0], args.hook_point, _n_heads)
            acts = acts.float()
            # Apply importance weighting: activation × output-projection norm
            if _importance_weights is not None:
                _l = my_layers[li]
                acts = acts * torch.from_numpy(_importance_weights[_l]).to(acts.device)
    ...
    if args.model_type == 'internvl':
        ...
        N_VIS = 512                                           # perceiver resampler: 64 tokens × up to 5 sub-images
    else:
    ...
    for sample_idx in tqdm(needed_samples, desc='Activation Pattern'):
        ...
        if args.model_type == 'internvl':
            ...
            _desc = descriptions.get(img_id, '') if descriptions else \
                    (captions_dict[img_id][0] if captions_dict else '')
            inputs, visual_positions, n_vis_this = prepare_inputs_idefics2(
                processor, img, _desc, device,
                image_token_id, args.text_source)
        elif _is_llava_llama3:
            _desc = descriptions.get(img_id, '') if descriptions else \
                    (captions_dict[img_id][0] if captions_dict else '')
            inputs, visual_positions, n_vis_this = prepare_inputs_llava_next(
                processor, img, _desc, device,
                image_token_id, args.text_source)
        elif args.model_type == 'llava-hf':
            inputs, visual_positions = prepare_inputs_hf(
        ...
        for li, nr_list in entries_by_layer.items():
            ...
            if isinstance(out, tuple):
                out = out[0]
            # For attn: reshape to per-head norms → (seq_len, n_heads)
            acts = _postprocess_acts(out[0], args.hook_point, _n_heads)
            acts = acts.float()
            # Apply importance weighting: activation × output-projection norm
            if _importance_weights is not None:
    ...
    if args.importance_weight:
        _hook_suffix += '_iw'
    out_base = os.path.join(args.output_dir, args.model, f'llm_fixed_threshold{_hook_suffix}{args.output_suffix}')
    ...
    config = {
        ...
        'scope': {'gate': 'llm_act_fn_output (SiLU(gate_proj(x)), gate only, per Xu refs [22,29])',
                  'gate_up': 'llm_gate_up_output (SiLU(gate_proj(x)) * up_proj(x), full intermediate)',
                  'attn': f'attn_o_proj_input (softmax(QK^T/sqrt(d))V, per-head L2 norms, '
                          f'{_n_heads} heads/layer)'}[args.hook_point],
    ...
    #   4. Classify based on p-values at α=0.001

    if args.skip_permutation:
        print(f'\n{"="*60}')
        print(f'PERMUTATION-TEST: SKIPPED (--skip_permutation flag set)')
        print(f'{"="*60}')
        print(f'Only fixed-threshold (FT) classification was written.')
        return
    ...
    perm_out_base = os.path.join(args.output_dir, args.model, f'llm_permutation{_hook_suffix}{args.output_suffix}')
...
def merge_results(args):
    ...
    if args.importance_weight:
        _hook_suffix += '_iw'
    if args.hook_point == 'gate_up':
        layer_names = get_down_proj_names(args.model_type, args.layer_end)
    elif args.hook_point == 'attn':
        layer_names = get_o_proj_names(args.model_type, args.layer_end)
    else:
    ...
    def _merge_dir(out_base, stats_glob, label_filename):
        ...
        # Adjust layer range to what was actually found
        effective_start = min(found_layers)
        effective_end = max(found_layers) + 1
        ...
        # Merge neuron labels — scan actual subdirectories instead of
        # constructing expected names (handles model.layers vs
        # model.language_model.layers vs language_model.model.layers)
        merged_labels = {}
        import re as _re
        for subdir in sorted(os.listdir(out_base)):
            subdir_path = os.path.join(out_base, subdir)
            if not os.path.isdir(subdir_path):
                continue
            label_path = os.path.join(subdir_path, label_filename)
            if not os.path.isfile(label_path):
                continue
            # Extract layer index from directory name (e.g. "model.layers.5.mlp.down_proj" → 5)
            m = _re.search(r'layers\.(\d+)\.', subdir)
            if m:
                l = int(m.group(1))
        ...
        expected_layers = set(per_layer_stats.keys())
        found_layers_labels = set(int(k) for k in merged_labels.keys())
        missing = expected_layers - found_layers_labels
        if missing:
            for l in sorted(missing):
                print(f'    WARNING: no {label_filename} for layer {l}')
    ...
    out_base_llm = os.path.join(args.output_dir, args.model, f'llm_fixed_threshold{_hook_suffix}{args.output_suffix}')
    ...
    out_base_perm = os.path.join(args.output_dir, args.model, f'llm_permutation{_hook_suffix}{args.output_suffix}')
