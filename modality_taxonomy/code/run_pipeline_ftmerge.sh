...
P5_FT=false                     # --p5-ft: use fixed-threshold labels (llm_fixed_threshold/neuron_labels_all.json) instead of PMBT permutation labels
...
if [[ "$P5_MODE" == "base" || "$P5_MODE" == "uniform" ]]; then
    ...
    $P5_FT && _P5_JOB_TAG="ft${_P5_JOB_TAG}"
fi
...
if [[ -f "$_MERGE_PTH" ]]; then
    ...
    if is_job_active "$_JOB"; then
        ...
        if [[ "$P5_MODE" == "pmbt" ]]; then
            ...
            # FT mode swaps PMBT permutation labels → fixed-threshold labels
            if $P5_FT; then _LBL_PREFIX="llm_fixed_threshold"; _LBL_FILE="neuron_labels_all.json"
            else _LBL_PREFIX="llm_permutation"; _LBL_FILE="neuron_labels_permutation_all.json"; fi
            if $P5_MIXED_HOOK; then
                # Mixed-hook: main labels are always gate_up
                _MLP_HOOK_DIR="${_LBL_PREFIX}_gate_up${_GEN_DIR_SUFFIX}"
            else
                _MLP_HOOK_DIR="${_LBL_PREFIX}${_HOOK_SUFFIX}${_GEN_DIR_SUFFIX}"
            fi

            # MLP labels (needed for scope=mlp or scope=both)
            if [[ "$P5_PMBT_SCOPE" == "mlp" || "$P5_PMBT_SCOPE" == "both" ]]; then
                _PMBT_LABELS="${_PMBT_BASE}/${_MLP_HOOK_DIR}/${_LBL_FILE}"
                if [[ ! -f "$_PMBT_LABELS" ]]; then
                    # Auto-detect: search for matching pattern
                    _PMBT_LABELS=$(find "$_PMBT_BASE" -path "*/${_MLP_HOOK_DIR}/${_LBL_FILE}" 2>/dev/null | head -1)
...
if $P5_EVAL; then
    ...
    if [[ ! -f "$_MERGE_PTH" ]]; then
        ...
        for _BN in "${_P5_BENCHES[@]}"; do
            ...
            _CMD="cd $WORK_DIR && export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True; set -a; source $_ENV_FILE 2>/dev/null || true; set +a; $_VE_PYTHON $_VE_DIR/run.py --data $_BN --model $_VE_MODEL --merge_model $_MERGE_PTH --judge $_BN_JUDGE --reuse --work-dir $_EVAL_DIR"
            if $LOCAL; then
