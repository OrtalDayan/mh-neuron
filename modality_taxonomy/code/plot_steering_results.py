...
def plot_multi_benchmark(data, best_alpha, output_dir, model_name,
    ...
    # Encoder/projector are alpha-independent (whole-component ablation).
    # They were only evaluated at alpha=0, so fall back to that when missing
    # at best_alpha to ensure their bars always appear in Figure C.
    fallback_dict = data['results'].get('0', {})
    alpha_independent = {'ablate_encoder', 'ablate_projector'}
    ...
    for i, condition in enumerate(conditions_to_plot):
        ...
        for bench_key in available_benchmarks:
            ...
            val = None
            # Use best_alpha results; fall back to alpha=0 for encoder/projector
            source = conditions_dict
            if condition not in source and condition in alpha_independent:
                source = fallback_dict
            if condition in source:
                cond_data = source[condition]
    ...
    ax.set_ylabel('Score')
    ax.set_ylim(0.0, 1.0)  # shared y-axis across all models for cross-figure comparability
...
def save_results_table(data, best_alpha, output_dir, model_name):
    ...
    # Same fallback as Figure C: encoder/projector only exist at alpha=0
    fallback_dict = data['results'].get('0', {})
    alpha_independent = {'ablate_encoder', 'ablate_projector'}
