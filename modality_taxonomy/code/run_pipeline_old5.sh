...
STEERING_ALPHAS="0.25,0.5,0.75,1.5,2.0"   # comma-separated alpha values
...
# For 'all' / 'all_att', clean all steps (1–11).
if [[ "$CLEAN_TO" == "auto" ]]; then
    case "$STEP" in
        all|all_att) CLEAN_TO=11 ;;
        *)           CLEAN_TO="$CLEAN_FROM" ;;
    esac
fi
...
# ═══════════════════════════════════════════════════════════════
# STEP 10 (halluc_score): POPE hallucination scoring per neuron
# ═══════════════════════════════════════════════════════════════
if [[ "$STEP" == "halluc_score" ]]; then

echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "  STEP 10: Hallucination scoring (POPE activations → Cohen's d + CETT)"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
STEP_LOG_DIR="${LOG_DIR}/10-halluc_score"
mkdir -p "$STEP_LOG_DIR" "$WORK_DIR/$STEP_LOG_DIR"

HALLUC_SCORES_DIR="results/10-halluc_scores/${MODE_DIR}/${MODEL_NAME}"

# ── Test mode: 2 layers, 20 POPE samples, 1 GPU ──────────────
if [[ "$MODE" == "test" ]]; then
    HS_LAYER_START=0
    HS_LAYER_END=2
    HS_POPE_SAMPLES=20
    HS_SHARDS=1
    echo "  [test] 2 layers, 20 POPE samples, 1 GPU"
else
    HS_LAYER_START=0
    HS_LAYER_END=$N_LAYERS
    HS_POPE_SAMPLES="${HALLUC_POPE_SAMPLES:-}"
    HS_SHARDS=$HALLUC_SCORE_SHARDS
    # Cap shards at number of layers (can't shard finer than 1 layer)
    if (( HS_SHARDS > N_LAYERS )); then
        HS_SHARDS=$N_LAYERS
    fi
    echo "  [full] ${N_LAYERS} layers, ${HS_SHARDS} GPU shards"
fi

HS_COMMON_ARGS="--model_type $MODEL_TYPE --original_model_path $MODEL_PATH"
...
if [[ -n "$HS_POPE_SAMPLES" ]]; then
    HS_COMMON_ARGS="$HS_COMMON_ARGS --num_pope_samples $HS_POPE_SAMPLES"
fi

if (( HS_SHARDS <= 1 )); then
    # Single GPU — run all layers
    JOB_NAME="${JN10}"
    echo "  → $JOB_NAME (layers ${HS_LAYER_START}–$((HS_LAYER_END-1)), 1 GPU)"
    rm -f "$WORK_DIR/$STEP_LOG_DIR/${JOB_NAME}${LOG_SUFFIX}.log" \
          "$WORK_DIR/$STEP_LOG_DIR/${JOB_NAME}${LOG_SUFFIX}.err"

    FULL_CMD="$PYTHON $HALLUC_SCORE_SCRIPT $HS_COMMON_ARGS \
        --layer_start $HS_LAYER_START --layer_end $HS_LAYER_END"
    ...
    # Multi-shard: split layer range across GPUs
    LAYERS_TOTAL=$((HS_LAYER_END - HS_LAYER_START))
    for (( s=0; s<HS_SHARDS; s++ )); do
        SHARD_START=$(( HS_LAYER_START + s * LAYERS_TOTAL / HS_SHARDS ))
        SHARD_END=$(( HS_LAYER_START + (s + 1) * LAYERS_TOTAL / HS_SHARDS ))
        SHARD_NAME="${JN10}_${s}"

        echo "  → $SHARD_NAME (layers ${SHARD_START}–$((SHARD_END-1)))"
        rm -f "$WORK_DIR/$STEP_LOG_DIR/${SHARD_NAME}${LOG_SUFFIX}.log" \
              "$WORK_DIR/$STEP_LOG_DIR/${SHARD_NAME}${LOG_SUFFIX}.err"
        ...
        bsub_tiered -q "$QUEUE" -J "$SHARD_NAME" \
            ...
            -- "cd $WORK_DIR && $PYTHON $HALLUC_SCORE_SCRIPT $HS_COMMON_ARGS \
                --layer_start $SHARD_START --layer_end $SHARD_END"
        SUBMITTED=$((SUBMITTED + 1))
    done
    echo "  → ${HS_SHARDS} shard jobs (tiers: ${GPU_GMEM_TIERS[*]})"
fi
...
# ── Test mode: 1 alpha, 3 conditions, no curve, minimal benchmarks ──
if [[ "$MODE" == "test" ]]; then
    ST_ALPHAS="0.5"
    ST_BASE_ARGS="--num_images 0 --conditions baseline ablate_vis random --pope_num_questions 5"
    ST_SHARDS=1
    ST_CHAIR_NUM=0
    [[ "$TRIVIAQA_NUM" == "2000" ]] && TRIVIAQA_NUM=5
    [[ "$MMLU_NUM" == "2000" ]]     && MMLU_NUM=5
    echo "  [test] alpha=0.5, 3 conditions, no curve, 1 GPU"
else
    ST_ALPHAS="$STEERING_ALPHAS"
    ST_BASE_ARGS="--conditions baseline ablate_vis ablate_text ablate_multimodal ablate_unknown random"
...
submit_steering_alpha() {
    ...
    local JOB_BASE="${JN11}_${JOB_SUFFIX}"
