...
def parse_args():
    ...
    p.add_argument('--ablation_scores_tqa', default=None,                  # Line 15b: pre-computed TriviaQA ΔH_TQA
                   help='Path to pre-computed TriviaQA ablation scores JSON; with '
                        '--skip_ablation this re-enables the text-side enrichment '
                        '(enrichment_results_tqa.json) in aggregation/10C mode')
    ...
    p.add_argument('--logit_pope_scoring', action='store_true',            # Line 18b: fast yes/no logit scoring
                   help='Score POPE yes/no via a single forward pass comparing the '
                        '"Yes"/"No" next-token logits instead of autoregressive '
...
def build_contrastive_pope_set(args, device='cuda:0'):
    ...
    if getattr(args, 'skip_contrastive_prep', False) and os.path.isfile(contrastive_path):
        ...
        # Reuse the CETT-diff already persisted by the prior full run instead of
        # recomputing it on GPU — avoids a ~1.5h contrastive re-sample in the
        # Phase-2 (10C) aggregation. Falls back to None if the file is absent.
        cett_diff = None                                                   # default: unavailable
        _cett_path = os.path.join(args.output_dir, 'cett_diff_scores.json')
        if os.path.isfile(_cett_path):
            with open(_cett_path) as f:
                _raw = json.load(f)                                        # {'L_N': score}
            cett_diff = {(int(k.split('_')[0]), int(k.split('_')[1])): v   # → {(L,N): score}
                         for k, v in _raw.items()}
        print(f'  [skip_contrastive_prep] Loaded existing {contrastive_path} '
              f'({n_clean} questions)'
              + (f' + {len(cett_diff)} saved CETT-diff scores' if cett_diff else '')
              + '. Skipping POPE rebuild.')
        return contrastive_path, n_clean, cett_diff
...
def build_contrastive_triviaqa_set(args, device='cuda:0'):
    ...
    if getattr(args, 'skip_contrastive_prep', False) and os.path.isfile(contrastive_path):
        with open(contrastive_path) as f:
            n_clean = sum(1 for line in f if line.strip())
        # Reuse the CETT-diff_TQA already persisted by the prior full run rather
        # than recomputing on GPU (mirrors the POPE fast path above).
        cett_diff_tqa = None                                               # default: unavailable
        _cett_path = os.path.join(args.output_dir, 'cett_diff_scores_tqa.json')
        if os.path.isfile(_cett_path):
            with open(_cett_path) as f:
                _raw = json.load(f)                                        # {'L_N': score}
            cett_diff_tqa = {(int(k.split('_')[0]), int(k.split('_')[1])): v
                             for k, v in _raw.items()}
        print(f'  [skip_contrastive_prep] Loaded existing {contrastive_path} '
              f'({n_clean} questions)'
              + (f' + {len(cett_diff_tqa)} saved CETT-diff_TQA scores' if cett_diff_tqa else '')
              + '. Skipping TriviaQA rebuild.')
        return contrastive_path, n_clean, cett_diff_tqa
...
def compute_hallucination_rate(model, processor, pope_questions,
    ...
    if logit_scoring or dense_scoring:                                     # Line 6b: build id tensors for the fast path
        ...
        if isinstance(_tok, tuple):                                       # Line 6d: llava-liuhaotian packs (tokenizer, img_proc)
            _tok = _tok[0]
        def _first_ids(_words):                                          # Line 6e: first sub-token id of each surface form
            _ids = set()
            for _w in _words:
                _enc = _tok.encode(_w, add_special_tokens=False)
                if _enc:
                    _ids.add(_enc[0])
            return sorted(_ids)
        ...
        if (logit_scoring or dense_scoring) and model_type in (            # Line 28a: HF-style backends only
            ...
            with torch.no_grad():                                          # Line 28b: one forward, no autoregressive loop
                if ablation_hook is not None:                              # Line 28c: apply ablation if scoring ablated
                    ...
                    logits = model(**inputs).logits
            next_logits = logits[0, -1, :]                                 # Line 28d: next-token logits at last position
            ...
            if (pred_yes and gt_answer == 'yes') or \
               (not pred_yes and gt_answer == 'no'):                       # Line 28h: correct if matches ground truth
                n_correct += 1
            if pred_yes and gt_answer == 'no':                            # Line 28i: hallucination = false "yes"
                n_hallucinated += 1
            n_total += 1                                                   # Line 28j: count this question
            continue                                                       # Line 28k: skip the generate/decode block
...
def main():
    ...
    if args.skip_ablation:                                                 # Line 6: use pre-computed scores
        ...
        neuron_scores_tqa = {}                                             # Line 8a: filled below if --ablation_scores_tqa given
        ...
        # Also load pre-computed TriviaQA ΔH_TQA when provided so the text-side
        # enrichment (enrichment_results_tqa.json) is emitted in skip-ablation
        # (Phase-2 / 10C) mode — otherwise it would be silently skipped.
        if getattr(args, 'ablation_scores_tqa', None) and os.path.isfile(args.ablation_scores_tqa):
            with open(args.ablation_scores_tqa) as f:
                tqa_raw = json.load(f)
            for key_str, val in tqa_raw.items():
                parts = key_str.split('_')
                neuron_scores_tqa[(int(parts[0]), int(parts[1]))] = val
            print(f'Loaded {len(neuron_scores_tqa)} pre-computed TriviaQA ΔH_TQA scores')
