...
Pipeline:
  Phase 0 — (Optional) Contrastive preprocessing (--contrastive / --halluc_triviaqa).

             POPE (--contrastive): Loads 3 POPE splits (random, popular,
             adversarial), samples N responses per question at temperature=0.7,
             keeps only questions where the model is consistently correct or
             consistently wrong.  Caps at 333 per split (~1000 total).
             Produces a high-confidence contrastive set for Phase 1.

             TriviaQA (--halluc_triviaqa): Loads the validation set, applies
             the same consistency filter across the full pool, and caps at
             1,000 clean examples (Section 3.5 dual-source design).
             POPE-derived ΔH should enrich VISUAL neurons.
             TriviaQA-derived ΔH_TQA should enrich TEXT neurons.
...
def parse_args():
    ...
    p.add_argument('--model_type', default='llava-liuhaotian',              # Line 2: which model backend to use
                   choices=['llava-hf', 'llava-liuhaotian', 'internvl',
                            ...
                            'llava-ov', 'idefics2', 'llava-llama3', 'llava-mistral'],
    ...
    # ── Per-layer LSF parallelism (optional) ──────────────────────
    p.add_argument('--layer_start', type=int, default=-1,                   # Line 6a: -1 = run all layers
                   help='First layer to ablate (inclusive). Default -1 means '
                        'run all layers via internal multi-GPU sharding. '
                        'When >=0, this job processes only layers [start,end) '
                        'on a single GPU and writes ablation_scores_layers'
                        '{start}-{end}.json (no enrichment computed).')
    p.add_argument('--layer_end', type=int, default=-1,                     # Line 6b: exclusive end
                   help='Last layer to ablate (exclusive). Pair with --layer_start.')
    p.add_argument('--skip_contrastive_prep', action='store_true',          # Line 6c: load existing
                   help='If set, load existing contrastive_pope.jsonl and '
                        'contrastive_triviaqa.jsonl from output_dir instead of '
                        'rebuilding. Required for per-layer LSF jobs after a '
                        'one-time setup job has built the contrastive sets.')
    ...
    # ── TriviaQA contrastive scoring (Section 3.5) ───────────────
    p.add_argument('--halluc_triviaqa', action='store_true',               # Line 16g: enable TriviaQA dual-source scoring
                   help='Enable TriviaQA-based text-hallucination scoring '
                        '(Section 3.5 dual-source design). Runs contrastive '
                        'consistency filtering on TriviaQA validation set, '
                        'computes ΔH_TQA per neuron, and runs enrichment '
                        '(should enrich text neurons).')
    p.add_argument('--triviaqa_path',                                      # Line 16h: path to TriviaQA verified-web JSON
                   default='data/triviaqa/qa/verified-web-dev.json',
                   help='Path to TriviaQA verified-web-dev.json')
    p.add_argument('--triviaqa_num', type=int, default=2000,               # Line 16i: pool size before filtering
                   help='Questions sampled from TriviaQA validation set '
                        'before consistency filtering (default: 2000)')
    p.add_argument('--triviaqa_cap', type=int, default=1000,               # Line 16j: max retained clean questions
                   help='Maximum clean TriviaQA examples retained after '
                        'consistency filtering (default: 1000)')
    ...
    p.add_argument('--halluc_measure', default='binary',                   # Line 18b: POPE hallucination signal
                   choices=['binary', 'logit_margin'],
                   help="How compute_hallucination_rate scores POPE. 'binary' "
                        "(default, original): generate text and count false-yes — "
                        "near-floor baselines make ΔH ~all-zero and the top-k "
                        "selection degenerates. 'logit_margin': soft false-yes rate "
                        "= mean P(yes)=sigmoid(logit_Yes-logit_No) over gt=no "
                        "questions, read from the answer-position logits via an "
                        "lm_head hook — moves smoothly under sub-threshold ablation "
                        "so ΔH is dense and tie-free.")
    ...
    # ── Resumable checkpointing (per-batch ΔH persistence) ─────────
    p.add_argument('--resume', dest='resume', action='store_true', default=True,  # Line 26: default = reuse checkpoints
                   help='Resume from per-batch checkpoints under '
                        '<output_dir>/checkpoints/ if present (default).')
    p.add_argument('--restart', dest='resume', action='store_false',        # Line 27: same dest → force recompute
                   help='Ignore and delete existing checkpoints; recompute the '
                        'ablation loop from scratch.')

    args = p.parse_args()
    # Alias new backends onto shared dispatch paths (Qwen2.5-VL→qwen2vl, LLaVA-Next-Mistral→llava-llama3).
    if args.model_type in ('qwen25vl-7b', 'qwen25vl-3b'):
        args.model_type = 'qwen2vl'
    elif args.model_type == 'llava-mistral':
        args.model_type = 'llava-llama3'
    return args
...
def get_layer_names(model_type, n_layers):
    ...
    if model_type == 'llava-hf':                                           # Line 1: HF LLaVA wrapper
        ...
        prefix = 'model.language_model.layers'
        suffix = 'mlp.act_fn'
    elif model_type == 'idefics2':                                         # Line 4a: Idefics2 (Mistral backbone, text_model wrapper)
        prefix = 'model.text_model.layers'
        suffix = 'mlp.act_fn'
    elif model_type == 'llava-llama3':                                     # Line 4b: LLaVA-Next-LLaMA3 (HF wrapper)
        prefix = 'model.language_model.layers'
        suffix = 'mlp.act_fn'
...
def build_contrastive_pope_set(args, device='cuda:0'):
    ...
    # ── Fast path: skip rebuild if --skip_contrastive_prep AND file exists ──
    contrastive_path = os.path.join(args.output_dir, 'contrastive_pope.jsonl')
    if getattr(args, 'skip_contrastive_prep', False) and os.path.isfile(contrastive_path):
        with open(contrastive_path) as f:
            n_clean = sum(1 for line in f if line.strip())
        print(f'  [skip_contrastive_prep] Loaded existing {contrastive_path} '
              f'({n_clean} questions). Skipping POPE rebuild.')
        return contrastive_path, n_clean, None  # cett_diff unavailable in skip mode
...
# ═══════════════════════════════════════════════════════════════════════
# Section 3 — TriviaQA helpers (Section 3.5 dual-source scoring)
# ═══════════════════════════════════════════════════════════════════════

def load_triviaqa_data(path, n, seed=42):
    """Load and subsample TriviaQA questions from verified-web-dev.json.

    TriviaQA's verified-web-dev.json has a top-level 'Data' key whose value
    is a list of question dicts.  Each dict has 'Question' and 'Answer',
    where Answer['NormalizedAliases'] is the list of accepted answer strings.

    Args:
        path: str — path to verified-web-dev.json
        n:    int — max questions to sample from the file
        seed: int — random seed for reproducibility

    Returns:
        list of dicts with keys 'question', 'aliases', 'question_id'
    """
    with open(path) as f:                                                  # Line 1: open JSON file
        raw = json.load(f)

    data = raw.get('Data', raw) if isinstance(raw, dict) else raw          # Line 2: handle dict or list format
    rng  = np.random.RandomState(seed)                                     # Line 3: reproducible sampling
    if len(data) > n:                                                      # Line 4: subsample if needed
        idx  = rng.choice(len(data), size=n, replace=False)
        data = [data[i] for i in sorted(idx)]                              # Line 5: preserve order

    questions = []                                                         # Line 6: normalised output list
    for entry in data:                                                     # Line 7: iterate raw entries
        q_text  = entry.get('Question', entry.get('question', ''))         # Line 8: question text
        answer  = entry.get('Answer',   entry.get('answer',   {}))          # Line 9: answer dict
        aliases = answer.get('NormalizedAliases',                          # Line 10: preferred normalised aliases
                             answer.get('Aliases', [answer.get('Value', '')]))
        aliases = [str(a).lower().strip() for a in aliases if a]           # Line 11: lowercase + strip
        questions.append({                                                 # Line 12: store entry
            'question':    q_text,
            'aliases':     aliases,
            'question_id': entry.get('QuestionId', entry.get('question_id', '')),
        })

    print(f'Loaded {len(questions)} TriviaQA questions from {path}')
    return questions                                                        # Line 13: return question list


def check_triviaqa_answer(generated, aliases):
    """Check whether a generated answer matches any TriviaQA alias.

    Normalises both sides (lowercase, strip punctuation) and uses substring
    containment — VLM outputs often include extra words around the answer.

    Args:
        generated: str — raw model output text
        aliases:   list[str] — acceptable normalised answer strings

    Returns:
        bool — True if any alias is found in the generated text
    """
    import re
    norm = re.sub(r'[^a-z0-9\s]', '', generated.lower()).strip()          # Line 1: normalise generated text
    for alias in aliases:                                                  # Line 2: check each alias
        a = re.sub(r'[^a-z0-9\s]', '', alias).strip()                     # Line 3: normalise alias identically
        if a and a in norm:                                                # Line 4: substring match
            return True
    return False                                                           # Line 5: no alias matched


def generate_triviaqa_answer(model, processor, question_text, model_type,
                             device, do_sample=True, temperature=0.7):
    """Generate a free-text answer for one TriviaQA question (text-only).

    Uses pure text input on every backend — no image is passed.
    The model is prompted to answer briefly in a few words.

    Args:
        model:         loaded VLM model
        processor:     tokenizer/processor
        question_text: str — the TriviaQA question
        model_type:    str — backend identifier
        device:        str — e.g. 'cuda:0'
        do_sample:     bool — True → temperature sampling; False → greedy
        temperature:   float — sampling temperature

    Returns:
        (generated: str, success: bool)
    """
    import torch                                                           # Line 1: torch for generation

    q = question_text + ' Answer briefly in a few words.'                 # Line 2: brevity prompt suffix

    gen_kw = dict(max_new_tokens=32, do_sample=do_sample)                  # Line 3: generation kwargs
    if do_sample:
        gen_kw['temperature'] = temperature                               # Line 4: only add temperature when sampling

    try:
        with torch.no_grad():                                              # Line 5: inference only
            if model_type == 'llava-hf':
                prompt = f'USER: {q}\nASSISTANT:'                        # Line 6: no <image> token — text-only prompt
                inputs = processor(text=prompt,
                                   return_tensors='pt').to(device)        # Line 7: no images= argument
                ...
                tok = processor[0] if isinstance(processor, tuple) else processor
                conv = conv_templates['v1'].copy()
                conv.append_message(conv.roles[0], q)                     # Line 8: no <image> placeholder
                conv.append_message(conv.roles[1], None)
                input_ids = tok(conv.get_prompt(),                         # Line 9: plain tokeniser — no image tokens
                                return_tensors='pt').input_ids.to(device)
                out = model.generate(input_ids=input_ids, **gen_kw)       # Line 10: no images= argument
                generated = tok.decode(out[0], skip_special_tokens=True)

            elif model_type == 'llava-ov':
                messages = [{'role': 'user', 'content': [
                    {'type': 'text', 'text': q}]}]                        # Line 11: text-only content — no image entry
                pt = processor.apply_chat_template(
                    messages, tokenize=False, add_generation_prompt=True)
                inputs = processor(text=pt,
                                   return_tensors='pt').to(device)        # Line 12: no images= argument
                out = model.generate(**inputs, **gen_kw)
                plen = inputs['input_ids'].shape[1]
                generated = processor.decode(
                    out[0][plen:], skip_special_tokens=True)               # Line 13: strip prompt tokens

            elif model_type == 'internvl':
                cfg = dict(max_new_tokens=32, do_sample=do_sample)
                if do_sample:
                    cfg['temperature'] = temperature
                generated = model.chat(processor, None, q, cfg)           # Line 14: pixel_values=None → text-only

            elif model_type == 'qwen2vl':
                messages = [{'role': 'user', 'content': [
                    {'type': 'text', 'text': q}]}]                        # Line 15: text-only content — no image entry
                pt = processor.apply_chat_template(
                    messages, tokenize=False, add_generation_prompt=True)
                inputs = processor(text=pt,
                                   return_tensors='pt').to(device)        # Line 16: no images= argument
                out = model.generate(**inputs, **gen_kw)
                plen = inputs['input_ids'].shape[1]
                generated = processor.decode(out[0][plen:], skip_special_tokens=True)

            elif model_type == 'idefics2':
                messages = [{'role': 'user', 'content': [
                    {'type': 'text', 'text': q}]}]                        # text-only content — no image entry
                pt = processor.apply_chat_template(
                    messages, tokenize=False, add_generation_prompt=True)
                inputs = processor(text=pt,
                                   return_tensors='pt').to(device)        # no images= argument
                out = model.generate(**inputs, **gen_kw)
                plen = inputs['input_ids'].shape[1]
                generated = processor.decode(out[0][plen:], skip_special_tokens=True)
                ...
                return '', False                                          # Line 17: unsupported model type

        return generated.strip(), True                                     # Line 18: return text + success flag

    except Exception:                                                      # Line 19: catch generation failures
        return '', False


def build_contrastive_triviaqa_set(args, device='cuda:0'):
    """Build the contrastive TriviaQA set and compute CETT-diff_TQA (Section 3.5).

    Mirrors build_contrastive_pope_set but for text-only factual questions.
    Samples K=10 responses per question at temperature=0.7 and retains only
    those where all K responses are unanimously correct or unanimously
    incorrect.  Caps at args.triviaqa_cap clean examples (default: 1,000).

    CETT-diff_TQA: computes mean(CETT | incorrect) − mean(CETT | correct)
    using a greedy forward pass on retained questions.  Text neurons should
    show higher CETT on questions the model gets wrong (text hallucination),
    giving positive CETT-diff for text-category neurons — the expected
    cross-validation signal described in Section 3.5.

    Outputs:
        <output_dir>/contrastive_triviaqa.jsonl  — retained question set
        <output_dir>/cett_diff_scores_tqa.json   — CETT-diff_TQA per neuron
    ...
    Returns:
        (contrastive_path: str, n_clean: int,
         cett_diff_tqa: dict {(layer, neuron): float} | None)
    """
    import torch                                                           # Line 1: torch for model ops
    from tqdm import tqdm                                                  # Line 2: progress bar

    # ── Fast path: skip rebuild if --skip_contrastive_prep AND file exists ──
    contrastive_path = os.path.join(args.output_dir, 'contrastive_triviaqa.jsonl')
    if getattr(args, 'skip_contrastive_prep', False) and os.path.isfile(contrastive_path):
        with open(contrastive_path) as f:
            n_clean = sum(1 for line in f if line.strip())
        print(f'  [skip_contrastive_prep] Loaded existing {contrastive_path} '
              f'({n_clean} questions). Skipping TriviaQA rebuild.')
        return contrastive_path, n_clean, None  # cett_diff_tqa unavailable in skip mode

    print(f'\n{"="*60}')
    print(f'TRIVIAQA CONTRASTIVE PREPROCESSING (Section 3.5)')
    print(f'  Pool: {args.triviaqa_num} questions sampled from validation set')
    print(f'  Consistency samples K: {args.contrastive_samples}')
    print(f'  Clean-example cap: {args.triviaqa_cap}')
    print(f'{"="*60}\n')

    # ── Load questions ──
    questions = load_triviaqa_data(                                        # Line 3: subsample from validation set
        args.triviaqa_path, args.triviaqa_num, seed=args.seed)

    # ── Load model ──
    print(f'  Loading model on {device} for TriviaQA sampling...')
    if args.model_type == 'llava-hf':
        from transformers import AutoProcessor, LlavaForConditionalGeneration
        processor = AutoProcessor.from_pretrained(args.model_path)
        model = LlavaForConditionalGeneration.from_pretrained(
        ...
        _LP   = os.path.join(_ROOT, 'LLaVA')
        if _LP not in sys.path: sys.path.insert(0, _LP)
        from llava.model.builder import load_pretrained_model
        from llava.mm_utils import get_model_name_from_path
        mn = get_model_name_from_path(args.model_path)
        tokenizer, model, image_processor, _ = load_pretrained_model(
            args.model_path, None, mn,
            device_map=device, torch_dtype=torch.float16)
        processor = (tokenizer, image_processor)
    elif args.model_type == 'llava-ov':
        from transformers import LlavaOnevisionForConditionalGeneration, AutoProcessor
    ...
    contrastive_qs = []                                                    # Line 4: accumulate clean questions
    stats = {'total': 0, 'consistent_correct': 0,
             'consistent_incorrect': 0, 'inconsistent': 0}               # Line 5: running statistics

    for q in tqdm(questions, desc='TriviaQA contrastive filter'):         # Line 6: iterate questions with progress
        aliases = q['aliases']
        if not aliases: continue                                           # Line 7: skip unanswerable questions

        preds, ok = [], True
        for _ in range(args.contrastive_samples):                          # Line 8: draw K responses
            gen, success = generate_triviaqa_answer(
                model, processor, q['question'],
                args.model_type, device,
                do_sample=True, temperature=0.7)
            if not success: ok = False; break                              # Line 9: abort on generation failure
            preds.append(check_triviaqa_answer(gen, aliases))              # Line 10: True = correct

        if not ok: continue                                                # Line 11: skip failed questions
        stats['total'] += 1                                                # Line 12: count evaluated

        all_correct   = all(preds)                                         # Line 13: K/K correct
        all_incorrect = all(not p for p in preds)                          # Line 14: 0/K correct

        if not (all_correct or all_incorrect):                             # Line 15: mixed → discard
            stats['inconsistent'] += 1; continue

        q['contrastive_label'] = 'correct' if all_correct else 'incorrect'  # Line 16: tag question
        if all_correct: stats['consistent_correct']   += 1
        else:           stats['consistent_incorrect'] += 1

        contrastive_qs.append(q)                                           # Line 17: add to clean set
        if len(contrastive_qs) >= args.triviaqa_cap:                       # Line 18: stop at cap
            print(f'  TriviaQA cap reached ({args.triviaqa_cap})')
            break

    # ── Save contrastive set ──
    os.makedirs(args.output_dir, exist_ok=True)
    contrastive_path = os.path.join(args.output_dir, 'contrastive_triviaqa.jsonl')
    with open(contrastive_path, 'w') as f:                                 # Line 19: write JSONL
        for q in contrastive_qs:
            f.write(json.dumps(q) + '\n')

    n_corr  = sum(1 for q in contrastive_qs if q['contrastive_label'] == 'correct')
    n_incor = sum(1 for q in contrastive_qs if q['contrastive_label'] == 'incorrect')
    print(f'\n  TriviaQA contrastive set: {len(contrastive_qs)} clean ')
    print(f'    {n_corr} consistently correct, {n_incor} consistently incorrect')
    print(f'  Filtering stats: {stats}')
    print(f'  Saved → {contrastive_path}')

    # ── Compute CETT-diff_TQA ──
    cett_diff_tqa = None                                                   # Line 20: default None
    try:
        from baukit import TraceDict                                       # Line 21: hook-based activation capture
        layer_names = get_layer_names(args.model_type, args.n_layers)      # Line 22: hook names for each layer

        incorrect_qs = [q for q in contrastive_qs
                        if q['contrastive_label'] == 'incorrect']          # Line 23: consistently wrong questions
        correct_qs   = [q for q in contrastive_qs
                        if q['contrastive_label'] == 'correct']            # Line 24: consistently right questions
        n_inc, n_cor = len(incorrect_qs), len(correct_qs)

        if n_inc >= 10 and n_cor >= 10:                                    # Line 25: need ≥10 per group for reliability
            import torch
            from PIL import Image
            cett_sum = {'incorrect': defaultdict(float),                   # Line 26: accumulate CETT sums per group
                        'correct':   defaultdict(float)}

            for label, group in [('incorrect', incorrect_qs),
                                  ('correct',   correct_qs)]:              # Line 27: process both groups
                for q in tqdm(group, desc=f'CETT-diff_TQA [{label}]'):
                    q_text = q['question'] + ' Answer briefly in a few words.'

                    # Build pure text inputs — no image involved
                    try:
                        if args.model_type == 'llava-ov':
                            msgs = [{'role': 'user', 'content': [
                                {'type': 'text', 'text': q_text}]}]        # Line 28: text-only content
                            pt = processor.apply_chat_template(
                                msgs, tokenize=False, add_generation_prompt=True)
                            inputs = processor(text=pt,
                                               return_tensors='pt').to(device)  # Line 29: no images= argument
                        elif args.model_type == 'llava-hf':
                            prompt = f'USER: {q_text}\nASSISTANT:'         # Line 30: no <image> token
                            inputs = processor(text=prompt,
                                               return_tensors='pt').to(device)  # Line 31: no images= argument
                        elif args.model_type == 'qwen2vl':
                            msgs = [{'role': 'user', 'content': [
                                {'type': 'text', 'text': q_text}]}]        # Line 32: text-only content
                            pt = processor.apply_chat_template(
                                msgs, tokenize=False, add_generation_prompt=True)
                            inputs = processor(text=pt,
                                               return_tensors='pt').to(device)  # Line 33: no images= argument
                        elif args.model_type == 'idefics2':
                            msgs = [{'role': 'user', 'content': [
                                {'type': 'text', 'text': q_text}]}]        # text-only content
                            pt = processor.apply_chat_template(
                                msgs, tokenize=False, add_generation_prompt=True)
                            inputs = processor(text=pt,
                                               return_tensors='pt').to(device)  # no images= argument
                            ...
                            tok = processor[0] if isinstance(processor, tuple) else processor
                            conv = conv_templates['v1'].copy()
                            conv.append_message(conv.roles[0], q_text)    # Line 34: no <image> placeholder
                            conv.append_message(conv.roles[1], None)
                            iids = tok(conv.get_prompt(),                  # Line 35: plain tokeniser
                                       return_tensors='pt').input_ids.to(device)
                            inputs = {'input_ids': iids}                  # Line 36: no images key
                        elif args.model_type == 'internvl':
                            # InternVL: forward pass via model() with pixel_values=None
                            tok = processor
                            enc = tok(q_text, return_tensors='pt').to(device)
                            inputs = {'input_ids': enc.input_ids,
                                      'pixel_values': None}               # Line 37: explicit None → text-only
                        else:
                            continue                                       # Line 38: skip unsupported backends
                    except Exception:
                        continue

                    with TraceDict(model, layer_names) as td:              # Line 29: attach activation hooks
                        with torch.no_grad():
                            if args.model_type == 'internvl':              # InternVL: run LM directly; full forward needs image_flags
                                model.language_model(input_ids=inputs['input_ids'])
                            else:
                                model(**inputs)                                 # Line 30: forward pass — activations recorded

                    last_pos = inputs['input_ids'].shape[1] - 1            # Line 31: last input token position

                    for l_idx, l_name in enumerate(layer_names):           # Line 32: iterate over layers
                        if l_name not in td: continue
                        act = td[l_name].output                            # Line 33: (1, seq_len, d_ffn)
                        if isinstance(act, tuple): act = act[0]
                        act_vec = act[0, last_pos, :].detach().float()     # Line 34: activations at last position

                        # Column norms of down-projection: ||W_down[:, j]||_2
                        try:
                            if args.model_type in ('llava-ov', 'qwen2vl', 'llava-hf', 'llava-llama3'):
                                wdown = model.model.language_model.layers[l_idx].mlp.down_proj.weight
                            elif args.model_type == 'idefics2':
                                wdown = model.model.text_model.layers[l_idx].mlp.down_proj.weight
                            elif args.model_type == 'internvl':
                                wdown = model.language_model.model.layers[l_idx].feed_forward.w2.weight
                            else:
                                wdown = model.model.layers[l_idx].mlp.down_proj.weight
                            col_norms = wdown.norm(dim=0).detach().float() # Line 35: (d_ffn,) column norms
                        except Exception:
                            col_norms = torch.ones(act_vec.shape[0])       # Line 36: fallback if weight path fails

                        cett = (act_vec.abs() * col_norms).cpu().numpy()   # Line 37: CETT_j = |a_j| × ||w_j||_2

                        for n_idx, val in enumerate(cett):                 # Line 38: accumulate into group
                            cett_sum[label][(l_idx, n_idx)] += float(val)

            # CETT-diff_TQA = mean(CETT | incorrect) − mean(CETT | correct)
            all_keys = set(cett_sum['incorrect']) | set(cett_sum['correct'])
            cett_diff_tqa = {
                key: cett_sum['incorrect'][key] / n_inc               # Line 39: normalise by group size
                     - cett_sum['correct'][key]   / n_cor
                for key in all_keys
            }

            # Save
            cett_tqa_path = os.path.join(args.output_dir, 'cett_diff_scores_tqa.json')
            ser = {f'{k[0]}_{k[1]}': v for k, v in cett_diff_tqa.items()}  # Line 40: serialise tuple keys
            with open(cett_tqa_path, 'w') as f:
                json.dump(ser, f, indent=2)
            print(f'  CETT-diff_TQA saved to {cett_tqa_path}')

            top10 = sorted(cett_diff_tqa.items(), key=lambda x: x[1], reverse=True)[:5]
            print(f'  Top-5 neurons by CETT-diff_TQA:')
            for (l, n), s in top10:
                print(f'    layer {l:2d} neuron {n:5d}: {s:.6f}')
        else:
            print(f'  WARNING: insufficient data for CETT-diff_TQA '
                  f'(incorrect={n_inc}, correct={n_cor}, need ≥10 each)')

    except Exception as e:                                                 # Line 41: graceful fallback
        print(f'  WARNING: CETT-diff_TQA computation failed: {e}')
        import traceback; traceback.print_exc()

    del model                                                              # Line 42: release model weights
    if __import__('torch').cuda.is_available():
        __import__('torch').cuda.empty_cache()                             # Line 43: free GPU cache
    print(f'  TriviaQA model unloaded.\n')

    return contrastive_path, len(contrastive_qs), cett_diff_tqa
...
def compute_triviaqa_error_rate(model, processor, tqa_questions,
    ...
    for q in tqa_questions:                                                # Line 6: iterate questions
        q_text  = q['question'] + ' Answer briefly in a few words.'       # Line 7: prompt with brevity instruction
        aliases = q['aliases']                                             # Line 8: acceptable answers

        # Build pure text inputs — no image on any backend
        if model_type == 'llava-hf':
            prompt = f'USER: {q_text}\nASSISTANT:'                       # Line 9: no <image> token
            inputs = processor(text=prompt,
                               return_tensors='pt').to(device)            # Line 10: no images= argument
        elif model_type == 'llava-liuhaotian':
            from llava.conversation import conv_templates
            tok = processor[0] if isinstance(processor, tuple) else processor
            conv = conv_templates['v1'].copy()
            conv.append_message(conv.roles[0], q_text)                    # Line 11: no <image> placeholder
            conv.append_message(conv.roles[1], None)
            iids = tok(conv.get_prompt(),
                       return_tensors='pt').input_ids.to(device)          # Line 12: plain tokeniser
            inputs = {'input_ids': iids}                                  # Line 13: no images key
        elif model_type == 'llava-ov':
            msgs = [{'role': 'user', 'content': [
                {'type': 'text', 'text': q_text}]}]                       # Line 14: text-only content
            pt = processor.apply_chat_template(
                msgs, tokenize=False, add_generation_prompt=True)
            inputs = processor(text=pt,
            ...
            msgs = [{'role': 'user', 'content': [
                {'type': 'text', 'text': q_text}]}]                       # Line 18: text-only content
            pt = processor.apply_chat_template(
                msgs, tokenize=False, add_generation_prompt=True)
            inputs = processor(text=pt,
                               return_tensors='pt').to(device)            # Line 19: no images= argument
        elif model_type == 'idefics2':
            msgs = [{'role': 'user', 'content': [
                {'type': 'text', 'text': q_text}]}]                       # text-only content
            pt = processor.apply_chat_template(
...
# ───────────────────────────────────────────────────────────────────────
# logit-margin helpers — used when --halluc_measure logit_margin.
# Single-neuron/batch ablation rarely flips a robust model's greedy yes/no,
# so the binary false-yes rate is ~all-zero and ΔH degenerates. Reading the
# answer-position Yes-vs-No logit margin gives a continuous signal that moves
# under sub-threshold ablation. Model-agnostic: an lm_head forward hook grabs
# the prefill logits from whatever generation path each backend already uses
# (including InternVL's model.chat), so no per-model image-input rebuild.
# ───────────────────────────────────────────────────────────────────────

def _resolve_lm_head(model):
    """Return the language-model head Linear, or None if not found."""
    if hasattr(model, 'lm_head'):                                          # Line 1: most HF causal-LM wrappers
        return model.lm_head
    lm = getattr(model, 'language_model', None)                            # Line 2: VLM nests the LM (e.g. InternVL)
    if lm is not None and hasattr(lm, 'lm_head'):
        return lm.lm_head
    for name, mod in model.named_modules():                                # Line 3: fallback — search by name
        if name.endswith('lm_head'):
            return mod
    return None                                                            # Line 4: caller falls back to binary


def _get_tokenizer(processor, model_type):
    """Extract the text tokenizer from a processor/tuple/tokenizer."""
    if isinstance(processor, tuple):                                       # Line 1: llava-liuhaotian → (tok, img_proc)
        return processor[0]
    if hasattr(processor, 'tokenizer'):                                    # Line 2: HF processors wrap a tokenizer
        return processor.tokenizer
    return processor                                                       # Line 3: internvl passes the tokenizer itself


def _yes_no_token_ids(tokenizer):
    """First-token ids for yes/no answer-word variants → (yes_ids, no_ids) sets."""
    yes_ids, no_ids = set(), set()                                         # Line 1: accumulate distinct ids
    variants = {'yes': ['Yes', 'yes', ' Yes', ' yes'],                     # Line 2: leading-space + case variants
                'no':  ['No',  'no',  ' No',  ' no']}
    for word, forms in variants.items():
        target = yes_ids if word == 'yes' else no_ids
        for form in forms:
            try:
                ids = tokenizer.encode(form, add_special_tokens=False)     # Line 3: tokenise without BOS/EOS
            except Exception:
                continue
            if ids:
                target.add(ids[0])                                         # Line 4: first sub-token carries the choice
...
def _prob_yes_from_logits(logits_row, yes_ids, no_ids):
    ...
    """
    import torch                                                          # Line 1: local import (GPU-side helper)
    yl = [logits_row[i] for i in yes_ids if i < logits_row.shape[0]]      # Line 2: candidate yes logits
    nl = [logits_row[i] for i in no_ids  if i < logits_row.shape[0]]      # Line 3: candidate no logits
    if not yl or not nl:                                                  # Line 4: token(s) missing → neutral
        return 0.5, False
    margin = torch.stack(yl).max() - torch.stack(nl).max()               # Line 5: best-yes minus best-no
    p_yes = torch.sigmoid(margin.float()).item()                         # Line 6: squash to (0,1)
    return p_yes, bool(margin.item() > 0)                                 # Line 7: soft score + hard prediction
...
class _AnswerLogitCapture:
    ...
    def __init__(self, lm_head):
        ...
        self.row = None                                                   # Line 1: (vocab,) logits at last prompt pos
        self._handle = None

    def __enter__(self):
        def hook_fn(module, inp, output):
            if self.row is not None:                                      # Line 2: keep only the first (prefill) call
                return
            out = output[0] if isinstance(output, tuple) else output     # Line 3: tensor of logits
            if out.dim() == 3:                                            # Line 4: (batch, seq, vocab) → last position
                self.row = out[0, -1, :].detach()
            elif out.dim() == 2:                                          # Line 5: (seq|batch, vocab) → last row
                self.row = out[-1, :].detach()
            else:
                self.row = out.detach().reshape(-1)                       # Line 6: already (vocab,)
...
def compute_hallucination_rate(model, processor, pope_questions,
                               ...
                               ablation_hook=None, halluc_measure='binary'):
    ...
    With halluc_measure='logit_margin', the binary false-yes count is replaced
    by a soft false-yes rate: mean over gt=no questions of P(yes) read from the
    answer-position Yes/No logit margin (dense, sub-threshold-sensitive ΔH).
    ...
    soft_halluc = 0.0                                                      # Line 5a: Σ P(yes) over gt=no (logit_margin)

    # logit_margin setup: resolve lm_head + yes/no token ids once. If the head
    # can't be found, silently fall back to the binary path for this call.
    use_lm = (halluc_measure == 'logit_margin')                           # Line 5b: requested mode
    lm_head = _resolve_lm_head(model) if use_lm else None                 # Line 5c: head for logit capture
    if use_lm and lm_head is None:                                        # Line 5d: no head → degrade gracefully
        print('  WARNING: --halluc_measure logit_margin but no lm_head found; '
              'falling back to binary for this evaluation')
        use_lm = False
    if use_lm:                                                            # Line 5e: precompute answer-word ids
        _yes_ids, _no_ids = _yes_no_token_ids(_get_tokenizer(processor, model_type))
        if not _yes_ids or not _no_ids:                                  # Line 5f: tokenizer lacks them → binary
            print('  WARNING: could not resolve yes/no token ids; '
                  'falling back to binary for this evaluation')
            use_lm = False
    ...
    for q in pope_questions:                                               # Line 7: iterate over POPE questions
        ...
        if model_type in ('llava-hf',):                                    # Line 14: HF LLaVA input format
            ...
            question_prompt = f'<image>\n{question_text}'
            # max_new_tokens=1 in logit_margin mode: only the prefill matters,
            # the lm_head hook captures the answer-position logits.
            generation_config = dict(max_new_tokens=1 if use_lm else 10,
                                     do_sample=False)
            cap = _AnswerLogitCapture(lm_head) if use_lm else None
            with torch.no_grad():
                _ctx_cap = cap if cap is not None else nullcontext()
            ...
            if use_lm and cap.row is not None:                             # logit_margin scoring
                p_yes, pred_yes = _prob_yes_from_logits(cap.row, _yes_ids, _no_ids)
                if gt_answer == 'no':
                    soft_halluc += p_yes
            ...
            inputs = processor(
                images=img, text=prompt_text,
                return_tensors='pt').to(device)
        elif model_type == 'idefics2':                                    # Idefics2 input format
            messages = [{'role': 'user', 'content': [
                {'type': 'image'},
                {'type': 'text', 'text': question_text},
            ]}]
            ...
            inputs = processor(
                images=img, text=prompt_text,
                return_tensors='pt').to(device)
        elif model_type == 'llava-llama3':                                # LLaVA-Next-LLaMA3 input format
            messages = [{'role': 'user', 'content': [
                {'type': 'image'},
                {'type': 'text', 'text': question_text},
            ]}]
        ...
        # Generate answer with optional ablation hook (+ logit capture).
        # In logit_margin mode only the prefill matters, so max_new_tokens=1.
        cap = _AnswerLogitCapture(lm_head) if use_lm else None             # Line 29a: per-question capture
        _max_new = 1 if use_lm else 10                                     # Line 29b: 1 token suffices for logits
        with torch.no_grad():                                              # Line 29: no gradient computation needed
            _ctx_cap = cap if cap is not None else nullcontext()           # Line 29c: capture logits if logit_margin
            _ctx_abl = ablation_hook if ablation_hook is not None else nullcontext()  # Line 30/31: ablate if requested
            with _ctx_cap, _ctx_abl:
                output_ids = model.generate(**inputs, max_new_tokens=_max_new,  # Line 32/34: generate
                                            do_sample=False)

        if use_lm and cap.row is not None:                                 # ── logit_margin scoring ──
            p_yes, pred_yes = _prob_yes_from_logits(cap.row, _yes_ids, _no_ids)
            if gt_answer == 'no':                                          # soft false-yes contribution
                soft_halluc += p_yes
        else:                                                              # ── binary scoring (original) ──
    ...
    # In logit_margin mode the hallucination rate is the SOFT false-yes rate
    # (Σ P(yes) over gt=no, ÷ n_total) — same scale/denominator as the binary
    # rate but continuous, so sub-threshold ablation yields dense, tie-free ΔH.
    if use_lm:                                                             # Line 44a: continuous rate
    ...
    return {                                                               # Line 46: return evaluation results
        ...
        'n_total': n_total,
        'measure': 'logit_margin' if use_lm else 'binary',                # Line 46a: which signal was used
        'soft_hallucination': soft_halluc if use_lm else None,            # Line 46b: Σ P(yes) over gt=no
    }


# ═══════════════════════════════════════════════════════════════════════
# Resumable checkpointing helpers — persist one JSON per (layer, batch)
# ablation unit so a killed / requeued job resumes without redoing GPU work.
# ═══════════════════════════════════════════════════════════════════════

def _checkpoint_dir(output_dir):
    """Directory holding one checkpoint JSON per completed ablation batch."""
    return os.path.join(output_dir, 'checkpoints')                          # Line 1: <output_dir>/checkpoints/


def _checkpoint_path(output_dir, layer_idx, batch_start):
    """Per-batch checkpoint filename: scores_layer{L}_batch{B}.json."""
    return os.path.join(_checkpoint_dir(output_dir),                        # Line 1: build full path
                        f'scores_layer{layer_idx}_batch{batch_start}.json')


def _atomic_write_json(path, data):
    """Write JSON to <path>.tmp then atomically replace <path>.

    os.replace is the atomic rename primitive: a crash mid-write leaves only
    the .tmp file, never a half-written final file a later resume would skip.
    """
    tmp = path + '.tmp'                                                     # Line 1: write to sibling temp file first
    with open(tmp, 'w') as f:                                              # Line 2: serialize fully to temp
        json.dump(data, f, indent=2)
    os.replace(tmp, path)                                                  # Line 3: atomic rename over final name


def _load_checkpoint(output_dir, layer_idx, batch_start):
    """Load a per-batch checkpoint → ({(L,n): ΔH}, {(L,n): ΔH_TQA}) tuple dicts."""
    with open(_checkpoint_path(output_dir, layer_idx, batch_start)) as f:   # Line 1: open the batch checkpoint
        cd = json.load(f)
    ns = {}                                                                # Line 2: POPE ΔH, tuple-keyed
    ns_tqa = {}                                                            # Line 3: TriviaQA ΔH_TQA, tuple-keyed
    for k, v in cd.get('neuron_scores', {}).items():                       # Line 4: deserialize "layer_neuron" keys
        p = k.split('_'); ns[(int(p[0]), int(p[1]))] = v
    for k, v in cd.get('neuron_scores_tqa', {}).items():                   # Line 5: deserialize TQA keys
        p = k.split('_'); ns_tqa[(int(p[0]), int(p[1]))] = v
    return ns, ns_tqa                                                      # Line 6: return both dicts


def _write_progress(output_dir, gpu_id, layer_idx, batch_start, batch_end, n_done):
    """Dump a tiny progress.json (last batch, timestamp, host) for monitoring."""
    import socket                                                          # Line 1: hostname for "which node am I on"
    prog = {                                                               # Line 2: minimal monitoring payload
        'last_layer': layer_idx,
        'last_batch_start': batch_start,
        'last_batch_end': batch_end,
        'batches_done_this_run': n_done,
        'gpu_id': gpu_id,
        'host': socket.gethostname(),
        'timestamp': time.strftime('%Y-%m-%dT%H:%M:%S'),
    }
    try:                                                                   # Line 3: monitoring is best-effort, never fatal
        _atomic_write_json(os.path.join(output_dir, 'progress.json'), prog)
    except Exception as _e:
        print(f'[GPU {gpu_id}] WARNING: progress.json write failed: {_e}')
...
def ablation_worker(gpu_id, layer_range, args, return_dict):
    ...
    """
    # ── Resume fast-path: if EVERY batch for this layer range is already
    #    checkpointed, assemble the result from disk and skip model load +
    #    all GPU ablation. (--restart deletes checkpoints up-front in main.) ──
    _resume = getattr(args, 'resume', True)                                # Line 0a: resume unless --restart given
    _batch_starts = list(range(0, args.n_neurons, args.batch_neurons))     # Line 0b: batch grid for this model
    if _resume:
        _all_done = all(                                                   # Line 0c: are all (layer,batch) units on disk?
            os.path.isfile(_checkpoint_path(args.output_dir, _L, _bs))
            for _L in range(layer_range[0], layer_range[1])
            for _bs in _batch_starts)
        if _all_done:
            _ns, _ns_tqa = {}, {}                                          # Line 0d: rebuild from checkpoints only
            for _L in range(layer_range[0], layer_range[1]):
                for _bs in _batch_starts:
                    _a, _b = _load_checkpoint(args.output_dir, _L, _bs)
                    _ns.update(_a); _ns_tqa.update(_b)
            return_dict[gpu_id] = {                                         # Line 0e: same shape as the compute path
                'neuron_scores': {f'{k[0]}_{k[1]}': v for k, v in _ns.items()},
                'neuron_scores_tqa': {f'{k[0]}_{k[1]}': v for k, v in _ns_tqa.items()},
                'baseline': None,                                          # already persisted by the original run
            }
            print(f'[GPU {gpu_id}] Resume: all {len(_ns)} neurons loaded from '
                  f'checkpoints — skipping model load + GPU ablation')
            return
    ...
    if args.model_type == 'llava-hf':                                      # Line 5: load HF LLaVA
        ...
        model = AutoModelForVision2Seq.from_pretrained(
            args.model_path, torch_dtype=torch.bfloat16,
            low_cpu_mem_usage=True).to(device).eval()
    elif args.model_type == 'idefics2':                                    # Idefics2 (Mistral backbone)
        from transformers import AutoModelForVision2Seq, AutoProcessor
        processor = AutoProcessor.from_pretrained(args.model_path)
        model = AutoModelForVision2Seq.from_pretrained(
            args.model_path, torch_dtype=torch.bfloat16,
            low_cpu_mem_usage=True).to(device).eval()
    elif args.model_type == 'llava-llama3':                                # LLaVA-Next-LLaMA3
        from transformers import AutoModelForVision2Seq, AutoProcessor
        processor = AutoProcessor.from_pretrained(args.model_path)
        model = AutoModelForVision2Seq.from_pretrained(
            args.model_path, torch_dtype=torch.bfloat16,
    ...
    baseline = compute_hallucination_rate(                                 # Line 19: evaluate without ablation
        ...
        halluc_measure=args.halluc_measure)
    ...
    if getattr(args, 'halluc_triviaqa', False):                            # Line 20c: check flag safely
        ...
        if os.path.isfile(tqa_contrastive_path):                           # Line 20e: file exists — load it
            with open(tqa_contrastive_path) as _f:
            ...
            print(f'[GPU {gpu_id}] WARNING: contrastive_triviaqa.jsonl not found — '
                  f'TriviaQA scoring skipped for this worker')

    # Per-neuron ablation scores for this GPU's layers
    neuron_scores     = {}                                                 # Line 21: POPE ΔH scores {(layer, neuron): delta_h}
    neuron_scores_tqa = {}                                                 # Line 21a: TriviaQA ΔH_TQA scores {(layer, neuron): delta_h_tqa}
    _n_batches_done   = 0                                                  # Line 21b: batches finished this run (for progress.json)
    os.makedirs(_checkpoint_dir(args.output_dir), exist_ok=True)           # Line 21c: ensure checkpoints/ exists before writes
    ...
    for layer_idx in range(layer_range[0], layer_range[1]):                # Line 22: iterate layers assigned to this GPU
        ...
        for batch_start in range(0, args.n_neurons, args.batch_neurons):   # Line 24: iterate neuron batches
            ...
            # Resume: if this (layer, batch) unit is already checkpointed,
            # load its scores from disk and skip the GPU ablation entirely.
            _ckpt = _checkpoint_path(args.output_dir, layer_idx, batch_start)  # Line 26a: this unit's checkpoint
            if _resume and os.path.isfile(_ckpt):                          # Line 26b: completed in a prior run?
                _a, _b = _load_checkpoint(args.output_dir, layer_idx, batch_start)
                neuron_scores.update(_a)                                   # Line 26c: merge POPE ΔH
                neuron_scores_tqa.update(_b)                               # Line 26d: merge TriviaQA ΔH_TQA
                continue                                                   # Line 26e: no GPU work for done units
            ...
            ablated = compute_hallucination_rate(                          # Line 46: evaluate with ablation active
                model, processor, pope_questions,
                args.pope_img_dir, device, args.model_type,
                ablation_hook=hook, halluc_measure=args.halluc_measure)

            delta_h = ablated['hallucination_rate'] - baseline_hr          # Line 47: compute ΔH (positive = more hallucinations)

            # Compute ΔH_TQA when TriviaQA questions are loaded
            delta_h_tqa = 0.0                                              # Line 47a: default 0 when TQA disabled
            if tqa_questions is not None and baseline_tqa_er is not None:  # Line 47b: TriviaQA scoring enabled
                ablated_tqa = compute_triviaqa_error_rate(                  # Line 47c: evaluate with same ablation hook
                    model, processor, tqa_questions, device, args.model_type,
            ...
            per_neuron_delta_tqa = delta_h_tqa / batch_size                # Line 48a: distribute ΔH_TQA equally across batch
            for n in range(batch_start, batch_end):                        # Line 49: store per-neuron scores
                neuron_scores[(layer_idx, n)]     = per_neuron_delta       # Line 49a: POPE-based ΔH
                neuron_scores_tqa[(layer_idx, n)] = per_neuron_delta_tqa   # Line 49b: TriviaQA-based ΔH_TQA

            # Persist this batch atomically so a crash / bsub -r requeue resumes here.
            _ckpt_data = {                                                 # Line 49c: self-contained per-batch payload
                'layer_idx': layer_idx,
                'batch_start': batch_start,
                'batch_end': batch_end,
                'neuron_scores': {f'{layer_idx}_{n}': per_neuron_delta
                                  for n in range(batch_start, batch_end)},
                'neuron_scores_tqa': {f'{layer_idx}_{n}': per_neuron_delta_tqa
                                      for n in range(batch_start, batch_end)},
            }
            _atomic_write_json(_ckpt, _ckpt_data)                          # Line 49d: tmp + atomic rename
            _n_batches_done += 1                                           # Line 49e: count for monitoring
            _write_progress(args.output_dir, gpu_id, layer_idx,            # Line 49f: refresh progress.json
                            batch_start, batch_end, _n_batches_done)
    ...
    return_dict[gpu_id] = {                                                # Line 51: pass results back to main process
        ...
        'neuron_scores_tqa': {f'{k[0]}_{k[1]}': v                         # Line 52a: ΔH_TQA scores (empty if TQA disabled)
                              for k, v in neuron_scores_tqa.items()},
...
def run_ablation_multi_gpu(args):
    ...
    neuron_scores     = {}                                                 # Line 20: merged POPE ΔH score dict
    neuron_scores_tqa = {}                                                 # Line 20a: merged TriviaQA ΔH_TQA score dict
    baseline = None
    for gpu_id in range(n_gpus):                                           # Line 21: iterate GPU results
        if gpu_id in return_dict:
            gpu_result = return_dict[gpu_id]
            for key_str, val in gpu_result['neuron_scores'].items():       # Line 22: deserialize POPE keys
                parts = key_str.split('_')
                neuron_scores[(int(parts[0]), int(parts[1]))] = val        # Line 23: reconstruct tuple keys
            for key_str, val in gpu_result.get('neuron_scores_tqa', {}).items():  # Line 23a: deserialize TQA keys
                parts = key_str.split('_')
                neuron_scores_tqa[(int(parts[0]), int(parts[1]))] = val    # Line 23b: reconstruct TQA tuple keys
            if baseline is None:
                baseline = gpu_result['baseline']                          # Line 24: grab baseline from first GPU

    print(f'\nMerged {len(neuron_scores)} neuron scores from {n_gpus} GPUs')
    if neuron_scores_tqa:
        print(f'  TriviaQA ΔH_TQA: {len(neuron_scores_tqa)} neurons scored')
    return neuron_scores, neuron_scores_tqa, baseline                      # Line 25: return merged results (now 3-tuple)
...
def compute_enrichment(flat_labels, neuron_scores, top_k_pct,
    ...
    # Single-neuron ablation ΔH on a binary task (e.g. POPE yes/no) almost never
    # flips an answer, so the score array can be ~all-zero. When the top-k
    # boundary falls inside a block of identical values, which neurons land
    # "inside" vs "outside" is decided by np.argsort's index order, NOT by ΔH —
    # the resulting odds ratios just reflect the modality mix of the highest-index
    # (last) layers, an artifact. Detect and flag that here.
    ...
    if degenerate:                                                         # warn loudly; numbers are not meaningful
        print(f'  ⚠️  WARNING: degenerate driving set — {n_at_thresh_in_drv:,}/{n_driving:,} '
              ...
              f'selection is decided by argsort index tie-break, not by ΔH; the '
              f'enrichment odds ratios from this score file are NOT meaningful '
              f'(likely an all-zero single-neuron ablation file — use the dense '
              f'combined/CETT-diff scores instead).')
...
def main():
    ...
    # ── Resumable checkpointing: prepare the checkpoints/ dir up-front ────
    # Default (--resume) keeps prior per-batch checkpoints so the ablation
    # loop skips completed units; --restart wipes them for a clean recompute.
    _ckpt_dir = _checkpoint_dir(args.output_dir)                           # Line 3a: <output_dir>/checkpoints/
    if getattr(args, 'resume', True):                                      # Line 3b: resume mode (default)
        if os.path.isdir(_ckpt_dir):
            print(f'[resume] Reusing checkpoints in {_ckpt_dir} '
                  f'({len(os.listdir(_ckpt_dir))} batch files present)')
    else:                                                                  # Line 3c: --restart → recompute from scratch
        if os.path.isdir(_ckpt_dir):
            import shutil
            shutil.rmtree(_ckpt_dir)
            print(f'[restart] Deleted checkpoint dir {_ckpt_dir}')
    os.makedirs(_ckpt_dir, exist_ok=True)                                  # Line 3d: ensure it exists before workers spawn
    ...
    cett_diff_scores     = None                                            # Line 5a: will hold POPE CETT-diff if contrastive
    cett_diff_scores_tqa = None                                            # Line 5a2: will hold TriviaQA CETT-diff_TQA
    if args.contrastive:                                                   # Line 5b: build contrastive POPE set + CETT-diff
    ...
    if args.skip_ablation:                                                 # Line 6: use pre-computed scores
        ...
        neuron_scores     = {}                                             # Line 8: reconstruct POPE score dict
        neuron_scores_tqa = {}                                             # Line 8a: no TQA scores when skipping ablation
    ...
    elif args.layer_start >= 0 and args.layer_end > args.layer_start:
        # ── Per-layer LSF mode: run single-GPU on a specific layer range ──
        import torch
        import torch.multiprocessing as mp
        print(f'\n{"="*60}')
        print(f'PER-LAYER MODE: ablating layers {args.layer_start}-{args.layer_end-1} '
              f'on a single GPU (cuda:0)')
        print(f'{"="*60}\n')
        manager = mp.Manager()
        return_dict = manager.dict()
        # Call worker directly without spawning — uses cuda:0
        ablation_worker(0, (args.layer_start, args.layer_end), args, return_dict)
        # Reconstruct dicts from worker output
        neuron_scores = {}
        neuron_scores_tqa = {}
        baseline = None
        if 0 in return_dict:
            r = return_dict[0]
            for key_str, val in r['neuron_scores'].items():
                parts = key_str.split('_')
                neuron_scores[(int(parts[0]), int(parts[1]))] = val
            for key_str, val in r.get('neuron_scores_tqa', {}).items():
                parts = key_str.split('_')
                neuron_scores_tqa[(int(parts[0]), int(parts[1]))] = val
            baseline = r.get('baseline')
        # Save with layer-range suffix
        suffix = f'_layers{args.layer_start}-{args.layer_end}'
        scores_path = os.path.join(args.output_dir, f'ablation_scores{suffix}.json')
        with open(scores_path, 'w') as f:
            json.dump({f'{k[0]}_{k[1]}': v for k, v in neuron_scores.items()},
                      f, indent=2)
        print(f'Per-layer ablation scores saved to {scores_path}')
        if neuron_scores_tqa:
            tqa_scores_path = os.path.join(args.output_dir,
                                           f'ablation_scores_tqa{suffix}.json')
            with open(tqa_scores_path, 'w') as f:
                json.dump({f'{k[0]}_{k[1]}': v for k, v in neuron_scores_tqa.items()},
                          f, indent=2)
            print(f'Per-layer TQA scores saved to {tqa_scores_path}')
        if baseline:
            baseline_path = os.path.join(args.output_dir,
                                         f'baseline_results{suffix}.json')
            with open(baseline_path, 'w') as f:
                json.dump(baseline, f, indent=2)
            print(f'Per-layer baseline saved to {baseline_path}')
        print(f'\n[Per-layer mode] Phase 2 (enrichment) skipped. '
              f'Run merge_halluc_scores.py after all per-layer jobs finish.')
        return  # Exit before Phase 2
    else:
        neuron_scores, neuron_scores_tqa, baseline = run_ablation_multi_gpu(args)  # Line 10: run multi-GPU ablation (3-tuple)
    ...
    # ── Save TriviaQA ablation scores (ΔH_TQA) ────────────────────
    if neuron_scores_tqa:                                                  # Line 21a: only if TQA ablation was run
        tqa_scores_path = os.path.join(args.output_dir, 'ablation_scores_tqa.json')
        tqa_ser = {f'{k[0]}_{k[1]}': v for k, v in neuron_scores_tqa.items()}  # Line 21b: serialise keys
        with open(tqa_scores_path, 'w') as f:
            json.dump(tqa_ser, f, indent=2)
        print(f'TriviaQA ablation scores (ΔH_TQA) saved to {tqa_scores_path}')

        # ── ΔH_TQA enrichment: text neurons should be enriched ───────
        print(f'\n{"="*60}')
        print(f'TRIVIAQA ΔH_TQA ENRICHMENT (Section 3.5 cross-validation)')
        print(f'{"="*60}\n')
        tqa_enrich = compute_enrichment(                                    # Line 21c: enrichment on TQA scores
            flat_labels, neuron_scores_tqa,
            top_k_pct=args.top_k_pct,
            n_random_trials=args.n_random_trials,
            alpha=args.alpha,
            seed=args.seed)
        ...
        print(f'\n{"─"*80}')
        print(f'ΔH_TQA ENRICHMENT — Top {args.top_k_pct}% by TriviaQA error-rate change')
        print(f'(Expected: TEXT neurons enriched — ablating them hurts text recall)')
        print(f'{"─"*80}')
        print(f'{"Category":<14} {"In Driving":>10} {"Expected":>10} '
              f'{"Fold":>8} {"OR":>8} {"p-value":>10} {"Direction":>10}')
        print(f'{"─"*80}')
        for cat in ['visual', 'text', 'multimodal', 'unknown']:
            r     = tqa_enrich['categories'][cat]
            or_s  = f'{r["odds_ratio"]:.2f}' if r["odds_ratio"] != 'inf' else 'inf'
            print(f'{cat:<14} {r["n_in_driving"]:>10,} {r["expected"]:>10.1f} '
                  f'{r["fold_enrichment"]:>8.3f} {or_s:>8} '
                  f'{r["p_value"]:>10.2e} {r["direction"]:>10}')
        print(f'{"─"*80}')

        tqa_enrich_path = os.path.join(args.output_dir, 'enrichment_results_tqa.json')
        with open(tqa_enrich_path, 'w') as f:
            json.dump(tqa_enrich, f, indent=2, default=str)
        print(f'TriviaQA enrichment saved to {tqa_enrich_path}')

        # ── CETT-diff_TQA enrichment (if available) ──────────────────
        if cett_diff_scores_tqa is not None:                               # Line 21d: CETT-diff_TQA enrichment
            print(f'\n{"="*60}')
            print(f'CETT-DIFF_TQA ENRICHMENT (independent of ablation)')
            print(f'{"="*60}\n')
            cett_tqa_enrich = compute_enrichment(
                flat_labels, cett_diff_scores_tqa,
                top_k_pct=args.top_k_pct,
                n_random_trials=args.n_random_trials,
                alpha=args.alpha,
                seed=args.seed)
            print(f'\n{"─"*80}')
            print(f'CETT-DIFF_TQA — Top {args.top_k_pct}%')
            print(f'{"─"*80}')
            print(f'{"Category":<14} {"In Driving":>10} {"Expected":>10} '
                  f'{"Fold":>8} {"OR":>8} {"p-value":>10} {"Direction":>10}')
            print(f'{"─"*80}')
            for cat in ['visual', 'text', 'multimodal', 'unknown']:
                r    = cett_tqa_enrich['categories'][cat]
                or_s = f'{r["odds_ratio"]:.2f}' if r["odds_ratio"] != 'inf' else 'inf'
                print(f'{cat:<14} {r["n_in_driving"]:>10,} {r["expected"]:>10.1f} '
                      f'{r["fold_enrichment"]:>8.3f} {or_s:>8} '
                      f'{r["p_value"]:>10.2e} {r["direction"]:>10}')
            print(f'{"─"*80}')
            cett_tqa_enrich_path = os.path.join(args.output_dir,
                                                 'enrichment_results_cett_diff_tqa.json')
            with open(cett_tqa_enrich_path, 'w') as f:
                json.dump(cett_tqa_enrich, f, indent=2, default=str)
            print(f'CETT-diff_TQA enrichment saved to {cett_tqa_enrich_path}')

        # ── Cross-validation summary: POPE vs TriviaQA enrichment fold ──
        print(f'\n{"="*60}')
        print(f'CROSS-VALIDATION SUMMARY (Section 3.5)')
        print(f'{"="*60}')
        print(f'{"Category":<14} {"POPE ΔH fold":>14} {"TQA ΔH_TQA fold":>16}')
        print(f'{"─"*50}')
        for cat in ['visual', 'text', 'multimodal', 'unknown']:
            pope_fold = results["categories"][cat]["fold_enrichment"]
            tqa_fold  = tqa_enrich["categories"][cat]["fold_enrichment"]
            print(f'{cat:<14} {pope_fold:>14.3f} {tqa_fold:>16.3f}')
        print(f'{"─"*50}')
        print(f'  Expected: POPE ΔH enriches VISUAL neurons')
        print(f'            TQA ΔH_TQA enriches TEXT neurons')
        print(f'{"="*60}\n')
