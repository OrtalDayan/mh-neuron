...
# ── Rename fig_*.pdf → fig_*_<model>.pdf so filenames are unique per model ──
echo "  → Renaming figures with model suffix '_${FIG_SUFFIX}'..."
for f in "$WORK_DIR/$PLOT_OUT_DIR"/fig_*.pdf; do
    [[ -f "$f" ]] || continue
    base="$(basename "$f" .pdf)"         # e.g. fig_A_dose_response
    newname="${base}_${FIG_SUFFIX}.pdf"   # e.g. fig_A_dose_response_llava_ov.pdf
...
def load_model(model_dir_name):
    """Load enrichment_results.json and per_layer_enrichment.npy for one model."""
    base = os.path.join(ENRICH_ROOT, model_dir_name)
    with open(os.path.join(base, "enrichment_results.json")) as f:
        er = json.load(f)
...
def plot_method_comparison(models_data_by_method, out_path):
    """
    ...
    for ax, (model_label, _) in zip(axes, MODELS):
        for mi, (method, mlabel, mcol) in enumerate(
                zip(methods, method_labels, method_colors)):
            er = models_data_by_method[model_label][method]
            folds = [er["categories"][cat]["fold_enrichment"] for cat in CATS]
    ...
    fig.suptitle("Three-Method Enrichment Comparison\n"
                 r"($\Delta H$, CETT-diff, Combined) — Visual Neuron Fold Enrichment",
                 fontsize=10, y=1.02)
    plt.tight_layout()
    plt.savefig(out_path, dpi=DPI, bbox_inches='tight')
    plt.close()
...
# Load all 3 method results for each model
method_files = {
    "dh":        "enrichment_results.json",
    "cett_diff": "enrichment_results_cett_diff.json",
    "combined":  "enrichment_results_combined.json",
}

models_data_by_method = {}
for label, dirname in MODELS:
    models_data_by_method[label] = {}
    base = os.path.join(ENRICH_ROOT, dirname)
    for method, fname in method_files.items():
        fpath = os.path.join(base, fname)
        with open(fpath) as f:
            models_data_by_method[label][method] = json.load(f)
    print(f"  {label}: all 3 method files loaded")
