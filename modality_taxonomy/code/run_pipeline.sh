...
MODE="full"            # test | full
SHARDS=32              # GPUs
MODEL_TYPE="llava-ov"    # model backend (llava-hf / llava-mistral / llava-llama3 / llava-ov / internvl / qwen2vl)
...
HOOK_POINT="gate_up"                                   # gate | gate_up | attn — hook point for classification
IMPORTANCE_WEIGHT=false                                # --importance-weight: weight activations by output-projection norms
FT_ONLY=false                                          # --ft-only: skip PMBT in step 3, skip PMBT idempotence checks in step 4 (Xu reproduction)
...
P5_MODE="pmbt"                   # base (BRV uniform) | pmbt (selective)
P5_TAXONOMY="pmbt"               # --p5-taxonomy: pmbt (permutation labels) | ft (Xu fixed-threshold labels) — masked merge in either case
P5_ALPHA="0.9"                   # default alpha (BRV uses 0.9)
P5_ALPHA_TEXT="0.9"              # alpha for text neurons (pmbt mode)
P5_ALPHA_VISUAL="1.0"            # alpha for visual neurons (pmbt mode)
P5_ALPHA_MULTI="1.0"             # alpha for multimodal neurons (pmbt mode)
P5_ALPHA_UNKNOWN="1.0"          # alpha for unknown/unlabeled neurons (pmbt mode); 1.0 = don't merge (only text MLP neurons merged)
P5_PMBT_SCOPE="mlp"             # mlp | attn | both — which neuron types to merge selectively
P5_KV_MERGE=false               # --p5-kv-merge: apply selective merge to k/v via GQA majority vote
P5_ALPHA_OTHER=""               # --p5-alpha-other: alpha for layernorms/embeddings (empty = use merge_pmbt.py default 1.0 = don't merge)
...
P5_MAX_NEW_TOKENS=""            # --p5-max-new-tokens: cap eval generation length (exports VLMEVAL_MAX_NEW_TOKENS); empty = model default
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --p5-taxonomy) P5_TAXONOMY="$2"; shift 2 ;;
        ...
        --p5-alpha-unknown) P5_ALPHA_UNKNOWN="$2"; shift 2 ;;
        ...
        --p5-max-new-tokens) P5_MAX_NEW_TOKENS="$2"; shift 2 ;;
...
# instead of --model-type llava-hf.  Full names still work.
_normalize_model_type() {
    case "$1" in
        llava|llava-1.5|llava-1.5-7b)  echo "llava-hf" ;;
...
# ── Validate model-type ─────────────────────────────────────────────────────
_VALID_MODELS="llava-hf llava-mistral llava-llama3 llava-ov internvl qwen2vl qwen25vl-7b qwen25vl-3b idefics2 all"
...
if [[ "$MODEL_TYPE" != *","* ]]; then
    ...
    if ! $_found; then
        ...
        echo ""
        echo "  Valid names:    llava-hf  llava-mistral  llava-llama3  llava-ov  internvl  qwen2vl  idefics2  all"
    ...
    for _cp in "${_check_parts[@]}"; do
        ...
        if ! $_found; then
            ...
            echo ""
            echo "  Valid names:    llava-hf  llava-mistral  llava-llama3  llava-ov  internvl  qwen2vl  idefics2"
            echo "  Short aliases:  llava             —         —         intern    qwen"
            exit 1
        fi
    done
...
ALL_MODELS=(llava-hf llava-mistral llava-llama3 llava-ov internvl qwen2vl qwen25vl-7b qwen25vl-3b idefics2)
...
if [[ "$MODEL_PATH" == "liuhaotian/llava-v1.5-7b" ]]; then          # not overridden by user
    if [[ "$MODEL_TYPE" == "internvl" ]]; then
        ...
        MODEL_PATH="modern_vlms/pretrained/Qwen2.5-VL-7B-Instruct"  # Qwen2.5-VL-7B
        ...
        MODEL_PATH="llava-hf/llava-1.5-7b-hf"                        # LLaVA-1.5-7B HF format
...
if ! $MODEL_NAME_SET && [[ "$MODEL_NAME" == "llava-1.5-7b" ]]; then       # not overridden by user
    ...
    # llava-hf keeps MODEL_NAME="llava-1.5-7b"
fi
...
if [[ "$MODEL_TYPE" == "qwen2vl" || "$MODEL_TYPE" == "qwen25vl-7b" || "$MODEL_TYPE" == "llava-ov" ]]; then
    N_LAYERS=28
elif [[ "$MODEL_TYPE" == "qwen25vl-3b" ]]; then
    N_LAYERS=36
fi
...
elif [[ "$MODEL_TYPE" == "llava-hf" || "$MODEL_TYPE" == "llava" || "$MODEL_TYPE" == "qwen2vl" || "$MODEL_TYPE" == "qwen25vl-7b" || "$MODEL_TYPE" == "qwen25vl-3b" || "$MODEL_TYPE" == "idefics2" || "$MODEL_TYPE" == "llava-ov" || "$MODEL_TYPE" == "llava-mistral" || "$MODEL_TYPE" == "llava-llama3" ]]; then
...
# ── FT-only flag (Xu reproduction: skip PMBT classification) ──────────────
# When set, neuron_modality_statistical.py writes only llm_fixed_threshold_*/
# and skips Section 8 entirely. The step-3 and step-4 idempotence checks
# below are also gated on $FT_ONLY so jobs aren't blocked waiting for PMBT.
$FT_ONLY && CLASSIFY_ARGS="--skip_permutation $CLASSIFY_ARGS"
...
case "$STEP" in
    ...
    halluc_score|halluc)      STEP="halluc_score" ;;
    ...
    steering|steer)             STEP="steering" ;;
    merge_steering)             STEP="merge_steering" ;;
    plot_steering|eccv)         STEP="plot_steering" ;;
    all|all_att)             ;;  # keep as-is
    *) echo "ERROR: unknown step '$STEP'"; echo "  Valid: 1-23, find_fig3, check_collisions, steering, merge_steering, plot_steering, all, all_att"; exit 1 ;;
esac
...
JN11="11_${SHORT_MODEL}"   # steering (activation steering with α-sweep)
...
if [[ "$MODE" == "test" || "$MODE" == "1-layer-100-imgs-test" || "$MODE" == "1-layer" ]]; then
  if [[ -n "$LAYER_LIST" ]]; then
    ...
    for ((s=0; s<CLASSIFY_SHARDS; s++)); do
        ...
        # Skip only if BOTH FT and PMBT exist (or FT only when --ft-only is set), or job is active
        if { [[ -s "$STATS_FILE" ]] && { [[ -s "$PMBT_FILE" ]] || $FT_ONLY; }; } || is_job_active "$JOB_NAME"; then
...
# In FT-only mode the PMBT merged file will never exist; only require FT.
if [[ -s "$MERGE_STATS_FILE" ]] && { [[ -s "$MERGE_PMBT_FILE" ]] || $FT_ONLY; } && [[ -z "${CLS_SUBMITTED_JOBS_ALL:-}" ]] && ! is_job_active "$MERGE_NC_JOB_CHK"; then
...
nl = {'llava-hf':32,'llava-llama3':32,'internvl':32,'idefics2':32,'qwen25vl-3b':36,'llava-ov':28,'qwen2vl':28,'qwen25vl-7b':28}
...
if [[ "$ATTN_HEATMAP_LAYERS" == "0 7 15 23 28 31" ]]; then
    if [[ "$MODEL_TYPE" == "llava-ov" || "$MODEL_TYPE" == "qwen2vl" || "$MODEL_TYPE" == "internvl" ]]; then
...
# ═══════════════════════════════════════════════════════════════
# STEP halluc_score: Per-neuron hallucination scoring (per-layer LSF fan-out)
# ───────────────────────────────────────────────────────────────
# Ported from submit_step10_phase1_fanout.sh. Each layer is one GPU job that
# runs halluc_score_neurons.py --layer_start L --layer_end L+1. Jobs are
# submitted with `bsub -r` so a dead host auto-requeues; on requeue the script
# resumes from its per-batch checkpoints in <output_dir>/checkpoints/
# (--resume is the default). Three phases:
#   Phase 0  layer 0       builds the contrastive POPE/TriviaQA sets + CETT-diff
#   Phase 1  layers 1..N-1  reuse them (--skip_contrastive_prep), wait on L0
#   Phase 2  10C (CPU/GPU)  merges per-layer ΔH + runs enrichment
# Invoked with:  bash code/run_pipeline.sh --step halluc_score [--model-type ...]
# ═══════════════════════════════════════════════════════════════
if [[ "$STEP" == "halluc_score" ]]; then

echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "  STEP halluc_score: per-neuron POPE ablation (per-layer fan-out) → enrichment"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
if [[ "$STEERING_TAXONOMY" == "ft" ]]; then
    _tax_subdir="llm_fixed_threshold"; HS_TAXONOMY="ft"
else
    _tax_subdir="llm_permutation";     HS_TAXONOMY="pmbt"
fi
# Default gate+pmbt keeps the bare path + job names so the clean dispatcher (~1145)
# and steering/SRF consumers (~5574,7473) are unaffected; other (hook,taxonomy)
# combos get a sibling subdir + a job-name suffix so they don't collide.
if [[ -z "$_HOOK_SUFFIX" && "$HS_TAXONOMY" == "pmbt" ]]; then
    _HS_COMBO_SUBDIR=""; _HS_COMBO_JOB=""
else
...
NEURONS_PER_LAYER=$(( N_LLM_NEURONS / N_LAYERS ))

# ── Resolve label dir to match classify output (hook + token-limit suffix). ──
# Prefer the suffixed dir (e.g. llm_permutation_gate_up_min100_max2048); fall back
# to the bare dir for older label sets.
_lbl="${_tax_subdir}${_HOOK_SUFFIX}${_GEN_DIR_SUFFIX}"
_full_labels="results/3-classify/full/$MODEL_NAME/$_lbl"
_mode_labels="results/3-classify/${CLASSIFY_MODE_DIR}/$MODEL_NAME/$_lbl"
_full_bare="results/3-classify/full/$MODEL_NAME/${_tax_subdir}${_HOOK_SUFFIX}"
if [[ -d "$WORK_DIR/$_full_labels" ]]; then
    HS_LABEL_DIR="$_full_labels"
elif [[ -d "$WORK_DIR/$_mode_labels" ]]; then
    HS_LABEL_DIR="$_mode_labels"
elif [[ -d "$WORK_DIR/$_full_bare" ]]; then
    HS_LABEL_DIR="$_full_bare"
else
    echo "  ERROR: No labels at $_full_labels (or bare $_full_bare) — run steps 1-4 first."
    exit 1
fi
echo "  Labels: $HS_LABEL_DIR  (taxonomy=$HS_TAXONOMY, hook=$HOOK_POINT)"
echo "  Output: $HALLUC_SCORES_DIR"

# ── Test vs full sizing ──
if [[ "$MODE" == "test" ]]; then
    HS_N_LAYERS=1                              # smoke test: layer 0 only
    HS_N_POPE=5
    HS_BATCH_NEURONS=50
else
    HS_N_LAYERS=$N_LAYERS                       # full: one GPU job per layer
    HS_N_POPE="${HALLUC_POPE_SAMPLES:-500}"
    HS_BATCH_NEURONS=50                         # matches halluc_score_neurons.py default
fi
echo "  [$MODE] layers=$HS_N_LAYERS  POPE=$HS_N_POPE  batch_neurons=$HS_BATCH_NEURONS  neurons/layer=$NEURONS_PER_LAYER"

# ── Args shared by every per-layer job ──
HS_COMMON_ARGS="--model_type $MODEL_TYPE --model_path $MODEL_PATH --model_name $MODEL_NAME"
HS_COMMON_ARGS="$HS_COMMON_ARGS --n_layers $N_LAYERS --n_neurons $NEURONS_PER_LAYER"
...
if $LOCAL; then
    # ── Smoke test: run layer 0 inline on this node (no bsub) ──
    echo "  [local] running layer 0 inline (builds contrastive sets + scores layer 0)"
    (cd "$WORK_DIR" && $PYTHON $HALLUC_SCORE_SCRIPT $HS_COMMON_ARGS \
        --layer_start 0 --layer_end 1) \
        2>&1 | tee "$STEP_LOG_DIR/10B_${SHORT_MODEL}_L0${LOG_SUFFIX}.log"
    echo "  [local] done — per-layer scores under $HALLUC_SCORES_DIR"
else

# ── Phase 0 + Phase 1: one GPU job per layer (bsub -r → resume on requeue) ──
HS_L0_JOB="10B_${SHORT_MODEL}${_HS_COMBO_JOB}_L0"
HS_PHASE_JOBS=""
for (( L=0; L<HS_N_LAYERS; L++ )); do
    JOB_NAME="10B_${SHORT_MODEL}${_HS_COMBO_JOB}_L${L}"
    _L_OUT="$WORK_DIR/$HALLUC_SCORES_DIR/ablation_scores_layers${L}-$((L+1)).json"
    if [[ -f "$_L_OUT" ]] || is_job_active "$JOB_NAME"; then
        echo "  [skip] $JOB_NAME — per-layer output exists or job active"
        SKIPPED=$((SKIPPED + 1))
        continue
    fi
    # Layer 0 builds the contrastive sets; layers >=1 reuse them and wait on L0.
    PHASE0_FLAG=""
    DEP_ARG=()
    if (( L >= 1 )); then
        PHASE0_FLAG="--skip_contrastive_prep"
        is_job_active "$HS_L0_JOB" && DEP_ARG=(-w "done($HS_L0_JOB)")
    fi
    rm -f "$WORK_DIR/$STEP_LOG_DIR/${JOB_NAME}${LOG_SUFFIX}.log" \
          "$WORK_DIR/$STEP_LOG_DIR/${JOB_NAME}${LOG_SUFFIX}.err"
    # -r : auto-requeue on host failure; the job then resumes from checkpoints.
    bsub -r -q "$QUEUE" -J "$JOB_NAME" "${DEP_ARG[@]}" \
         -gpu "num=1:gmem=40G" -R "rusage[mem=65536]" \
         -oo "$WORK_DIR/$STEP_LOG_DIR/${JOB_NAME}${LOG_SUFFIX}.log" \
         -eo "$WORK_DIR/$STEP_LOG_DIR/${JOB_NAME}${LOG_SUFFIX}.err" \
         "cd $WORK_DIR && $PYTHON $HALLUC_SCORE_SCRIPT $HS_COMMON_ARGS --layer_start $L --layer_end $((L+1)) $PHASE0_FLAG"
    echo "  → $JOB_NAME (layer $L)${DEP_ARG:+  [waits on $HS_L0_JOB]}"
    HS_PHASE_JOBS="${HS_PHASE_JOBS:+$HS_PHASE_JOBS }$JOB_NAME"
    SUBMITTED=$((SUBMITTED + 1))
done

# ── Phase 2: merge per-layer ΔH + run enrichment (waits on all 10B jobs) ──
# Needs a GPU when contrastive is on: --skip_contrastive_prep returns CETT-diff
# = None, so the enrichment pass recomputes CETT-diff on GPU (it is NOT passed).
HS_AGG_JOB="10C_${SHORT_MODEL}${_HS_COMBO_JOB}"
HS_ENR_DONE="$WORK_DIR/$HALLUC_SCORES_DIR/enrichment_results.json"
if { [[ -z "$HS_PHASE_JOBS" ]] && [[ -f "$HS_ENR_DONE" ]]; } || is_job_active "$HS_AGG_JOB"; then
    echo "  [skip] $HS_AGG_JOB — enrichment exists (nothing re-submitted) or job active"
else
    AGG_DEP=()
    if [[ -n "$HS_PHASE_JOBS" ]]; then
        _dep=""
        for j in $HS_PHASE_JOBS; do _dep="${_dep:+$_dep && }done($j)"; done
        AGG_DEP=(-w "$_dep")
    fi
    AGG_GPU=()
    [[ "$HALLUC_CONTRASTIVE" == "1" ]] && AGG_GPU=(-gpu "num=1:gmem=40G" -R "rusage[mem=65536]")
    HS_ENR_ARGS="--model_type $MODEL_TYPE --model_path $MODEL_PATH --model_name $MODEL_NAME"
    HS_ENR_ARGS="$HS_ENR_ARGS --n_layers $N_LAYERS --n_neurons $NEURONS_PER_LAYER"
    HS_ENR_ARGS="$HS_ENR_ARGS --label_dir $HS_LABEL_DIR --taxonomy $HS_TAXONOMY"
    HS_ENR_ARGS="$HS_ENR_ARGS --pope_path $POPE_PATH --pope_img_dir $POPE_IMG_DIR"
    [[ "$HALLUC_CONTRASTIVE" == "1" ]] && HS_ENR_ARGS="$HS_ENR_ARGS --contrastive"
    # Phase 1 already built the contrastive set + saved CETT-diff to disk; have
    ...
    bsub -q "$QUEUE" -J "$HS_AGG_JOB" "${AGG_DEP[@]}" "${AGG_GPU[@]}" \
         -oo "$WORK_DIR/$STEP_LOG_DIR/${HS_AGG_JOB}${LOG_SUFFIX}.log" \
         -eo "$WORK_DIR/$STEP_LOG_DIR/${HS_AGG_JOB}${LOG_SUFFIX}.err" \
         "cd $WORK_DIR && $PYTHON code/merge_halluc_scores.py --output_dir $HALLUC_SCORES_DIR --n_layers $N_LAYERS --run_enrichment --halluc_score_script $HALLUC_SCORE_SCRIPT --python $PYTHON --enrichment_args $HS_ENR_ARGS"
    echo "  → $HS_AGG_JOB (Phase 2: merge per-layer ΔH + enrichment)"
    SUBMITTED=$((SUBMITTED + 1))
fi

fi  # end local vs bsub

fi  # end step halluc_score
...
if [[ -z "$MERGE_BASE_LLM_PATH" ]]; then
    case "$MODEL_TYPE" in
        ...
        llava-hf) MERGE_BASE_LLM_PATH="NousResearch/Llama-2-7b-hf" ;;
    esac
...
if [[ -z "$MERGE_MATH_LLM_PATH" ]]; then
    case "$MODEL_TYPE" in
        ...
        llava-hf) MERGE_MATH_LLM_PATH="meta-math/MetaMath-7B-V1.0" ;;
    esac
    echo "  [auto] math LLM: $MERGE_MATH_LLM_PATH"
fi
...
if [[ "$MODEL_TYPE" == "llava-hf" || "$MODEL_TYPE" == "llava" ]]; then
...
STEP24_LABEL_DIR="$OUTPUT_DIR/$MODEL_NAME/llm_permutation${_HOOK_SUFFIX}${_GEN_DIR_SUFFIX}"
...
# Check if labels exist (skip for baseline-only)
if ! $P4_BASELINE_ONLY && [[ ! -d "$WORK_DIR/$STEP24_LABEL_DIR" ]]; then
    echo "  ERROR: Label dir not found: $STEP24_LABEL_DIR"
...
if [[ -z "$MERGE_MATH_LLM_PATH" ]]; then
    case "$MODEL_TYPE" in
        ...
        llava-hf)
            MERGE_MATH_LLM_PATH="meta-math/MetaMath-7B-V1.0"
            echo "  [auto] math LLM: MetaMath-7B-V1.0 (strongest LLaMA-2-7B math SFT)"
            ;;
        llava-ov)
            MERGE_MATH_LLM_PATH="Qwen/Qwen2-Math-7B"
            echo "  [auto] math LLM: Qwen2-Math-7B (LLaVA-OneVision donor)"
            ;;
        internvl)
            MERGE_MATH_LLM_PATH="internlm/internlm2-math-plus-7b"
            echo "  [auto] math LLM: internlm2-math-plus-7b (InternVL donor)"
            ;;
        qwen25vl-7b)
            MERGE_MATH_LLM_PATH="Qwen/Qwen2.5-Math-7B"
            echo "  [auto] math LLM: Qwen2.5-Math-7B (Qwen2.5-VL donor)"
            ;;
        qwen25vl-3b)
            # Only 36-layer shape-compatible math donor (official Qwen2.5-Math is 28-layer).
            # Community DAPO RL fine-tune of Qwen2.5-3B: GSM8K 86.5%, MATH-500 70.7%.
            MERGE_MATH_LLM_PATH="jaygala24/Qwen2.5-3B-DAPO-math-reasoning"
            echo "  [auto] math LLM: Qwen2.5-3B-DAPO-math-reasoning (36-layer shape-compatible donor)"
            ;;
        llava-mistral)
            MERGE_MATH_LLM_PATH="hkust-nlp/dart-math-mistral-7b-uniform"
            echo "  [auto] math LLM: dart-math-mistral-7b-uniform (Dart-Math, BRV best for LLaVA-Mistral; mirrors steps 13-18 donor)"
            ;;
    esac
fi
...
_MATH_LLM_SHORT=$(basename "$MERGE_MATH_LLM_PATH" | sed \
    ...
    -e 's/Qwen2\.5-Math-7B/qwen25-math/' \
    -e 's/Qwen2\.5-3B-DAPO-math-reasoning/dapo-math-3b/' \
...
if [[ "$P5_MODE" == "base" || "$P5_MODE" == "uniform" ]]; then
    ...
    # Taxonomy selects which classifier labels feed the masked merge:
    #   pmbt = permutation labels (llm_permutation*), ft = Xu fixed-threshold (llm_fixed_threshold*)
    if [[ "$P5_TAXONOMY" == "ft" ]]; then
        _TAX_BASE="llm_fixed_threshold"; _TAX_FILE="neuron_labels_all.json"
        _MODE_PREFIX="ft_pmbt_"; _TAX_JOB="ft"
    else
        _TAX_BASE="llm_permutation"; _TAX_FILE="neuron_labels_permutation_all.json"
        _MODE_PREFIX="pmbt_"; _TAX_JOB=""
    fi
    _SCOPE_SUFFIX=""
    ...
    [[ "$P5_ALPHA_UNKNOWN" != "1.0" ]] && _SCOPE_SUFFIX="${_SCOPE_SUFFIX}_u${P5_ALPHA_UNKNOWN}"
    ...
    _MODE_DIR="${_MODE_PREFIX}t${P5_ALPHA_TEXT}_v${P5_ALPHA_VISUAL}_m${P5_ALPHA_MULTI}${_SCOPE_SUFFIX}"
fi
...
if [[ "$P5_MODE" == "base" || "$P5_MODE" == "uniform" ]]; then
    ...
    _P5_JOB_TAG="${_TAX_JOB}pt${_at}v${_av}m${_am}${_scope_tag}"
fi
...
case "$MODEL_TYPE" in
    ...
    llava-hf)      _VE_MODEL="llava_v1.5_7b_hf" ;;
    llava-mistral) _VE_MODEL="llava_next_mistral_7b" ;;
    llava-ov)      _VE_MODEL="llava-onevision-qwen2-7b-ov-hf" ;;
    internvl)      _VE_MODEL="InternVL2_5-8B" ;;
    qwen25vl-7b)   _VE_MODEL="Qwen2.5-VL-7B-Instruct" ;;
    qwen25vl-3b)   _VE_MODEL="Qwen2.5-VL-3B-Instruct" ;;
esac

# OpenAI API key is sourced from this .env INSIDE each job (never inlined into the
# bsub command string — inlining leaks the key into the LSBATCH echo / job logs).
_ENV_FILE="$WORK_DIR/modern_vlms/VLMEvalKit/.env"
_VE_PYTHON="$WORK_DIR/modern_vlms/VLMEvalKit_brv/.venv/bin/python"
...
if $P5_BASELINE; then
    ...
    for _BN in "${_P5_BENCHES[@]}"; do
        ...
        _SCORE=$(find "$_BL_OUT" \( -name "*${_BN}*_score.csv" -o -name "*${_BN}*_acc.csv" \) 2>/dev/null | head -1 || true)
        if [[ -n "$_SCORE" ]]; then
        ...
        _BN_JUDGE="$P5_JUDGE"
        [[ "$_BN" == "MMStar" || "$_BN" == "MME" || "$_BN" == "POPE" || "$_BN" == "AI2D_TEST" ]] && _BN_JUDGE="exact_matching"  # HallusionBench uses GPT-assisted extraction (matches paper/leaderboard)
        ...
        _CMD="cd $WORK_DIR && export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True; set -a; source $_ENV_FILE 2>/dev/null || true; set +a; $_VE_PYTHON $_VE_DIR/run.py --data $_BN --model $_VE_MODEL --judge $_BN_JUDGE --reuse --work-dir $_BL_OUT"
        if $LOCAL; then
    ...
    if is_job_active "$_JOB"; then
        ...
        if [[ "$P5_MODE" == "pmbt" ]]; then
            ...
            if $P5_MIXED_HOOK; then
                ...
                _MLP_HOOK_DIR="${_TAX_BASE}_gate_up${_GEN_DIR_SUFFIX}"
            else
                _MLP_HOOK_DIR="${_TAX_BASE}${_HOOK_SUFFIX}${_GEN_DIR_SUFFIX}"
            fi

            # MLP labels (needed for scope=mlp or scope=both)
            if [[ "$P5_PMBT_SCOPE" == "mlp" || "$P5_PMBT_SCOPE" == "both" ]]; then
                _PMBT_LABELS="${_PMBT_BASE}/${_MLP_HOOK_DIR}/${_TAX_FILE}"
                if [[ ! -f "$_PMBT_LABELS" ]]; then
                    # Auto-detect: search for matching pattern
                    _PMBT_LABELS=$(find "$_PMBT_BASE" -path "*/${_MLP_HOOK_DIR}/${_TAX_FILE}" 2>/dev/null | head -1)
                ...
                if $P5_MIXED_HOOK; then
                    _GATE_DIR="${_TAX_BASE}${_GEN_DIR_SUFFIX}"
                    _PMBT_GATE="${_PMBT_BASE}/${_GATE_DIR}/${_TAX_FILE}"
                    if [[ ! -f "$_PMBT_GATE" ]]; then
                        _PMBT_GATE=$(find "$_PMBT_BASE" -path "*/${_GATE_DIR}/${_TAX_FILE}" 2>/dev/null | head -1)
                    fi
            ...
            _MERGE_CMD="$_MERGE_CMD \
                ...
                --alpha_unknown $P5_ALPHA_UNKNOWN"
            ...
            if [[ "$P5_PMBT_SCOPE" == "attn" || "$P5_PMBT_SCOPE" == "both" ]]; then
                _PMBT_ATTN="${_PMBT_BASE}/${_TAX_BASE}_attn${_GEN_DIR_SUFFIX}/${_TAX_FILE}"
                if [[ ! -f "$_PMBT_ATTN" ]]; then
                    _PMBT_ATTN=$(find "$_PMBT_BASE" -path "*/${_TAX_BASE}_attn*/${_TAX_FILE}" 2>/dev/null | head -1 || true)
                fi
...
if $P5_EVAL; then
    ...
    if [[ ! -f "$_MERGE_PTH" ]]; then
        ...
        for _BN in "${_P5_BENCHES[@]}"; do
            ...
            _SCORE=$(find "$_EVAL_DIR" \( -name "*${_BN}*_score.csv" -o -name "*${_BN}*_acc.csv" \) 2>/dev/null | head -1 || true)
            if [[ -n "$_SCORE" ]]; then
                echo "  [skip] $_JOB — score exists"
                SKIPPED=$((SKIPPED + 1))
                continue
            ...
            _MNT_ENV=""; [[ -n "$P5_MAX_NEW_TOKENS" ]] && _MNT_ENV="export VLMEVAL_MAX_NEW_TOKENS=$P5_MAX_NEW_TOKENS; "
            _CMD="cd $WORK_DIR && export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True; set -a; source $_ENV_FILE 2>/dev/null || true; set +a; ${_MNT_ENV}$_VE_PYTHON $_VE_DIR/run.py --data $_BN --model $_VE_MODEL --merge_model $_MERGE_PTH --judge $_BN_JUDGE --reuse --work-dir $_EVAL_DIR"
            if $LOCAL; then
...
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "  STEP 11: Activation steering (alpha sweep × conditions)"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
STEP_LOG_DIR="${LOG_DIR}/11-steering"
mkdir -p "$STEP_LOG_DIR" "$WORK_DIR/$STEP_LOG_DIR"

LABELS_BASE="${OUTPUT_DIR_USER:-$OUTPUT_DIR}"
STEERING_OUT="results/11-steering/${CLASSIFY_MODE_DIR}/${MODEL_NAME}/${HALLUC_SCORE_METHOD}"
...
_TQA_CHECK="results/10-halluc_scores/${CLASSIFY_MODE_DIR}/${MODEL_NAME}/ablation_scores_delta_h_tqa.json"
...
HALLUC_SCORES_DIR_ST="results/10-halluc_scores/${CLASSIFY_MODE_DIR}/${MODEL_NAME}"
...
STEERING_OUT="results/11-steering/${CLASSIFY_MODE_DIR}/${MODEL_NAME}/${HALLUC_SCORE_METHOD}"
PLOT_OUT_DIR="results/13-plots/${CLASSIFY_MODE_DIR}/${MODEL_NAME}${RUN_SUFFIX}/${HALLUC_SCORE_METHOD}"
...
PLOT_OUT_DIR="results/13-plots/${CLASSIFY_MODE_DIR}/${MODEL_NAME}${RUN_SUFFIX}/${HALLUC_SCORE_METHOD}"
MERGED_JSON="$PLOT_OUT_DIR/steering_merged_${HALLUC_SCORE_METHOD}.json"
HALLUC_SCORES_DIR_PLOT="results/10-halluc_scores/${CLASSIFY_MODE_DIR}/${MODEL_NAME}${RUN_SUFFIX}"

if [[ ! -f "$WORK_DIR/$MERGED_JSON" ]]; then
    echo "  ERROR: $MERGED_JSON not found."
    echo "         Run step 12 first: bash run_pipeline.sh --step merge_steering"
    exit 1
fi
