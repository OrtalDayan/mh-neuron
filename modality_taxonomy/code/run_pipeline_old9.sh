...
if [[ "$MODE" == "test" ]]; then
    ...
    HS_N_LAYERS=1
    HS_N_GPUS=1
    echo "  [test] $HS_N_POPE POPE questions, $HS_N_LAYERS layer, $HS_N_GPUS GPU, taxonomy=$HS_TAXONOMY"
else
    HS_N_POPE="${HALLUC_POPE_SAMPLES:-500}"
    HS_N_LAYERS=$N_LAYERS
    HS_N_GPUS=$HALLUC_SCORE_SHARDS
    echo "  [full] ${HS_N_POPE} POPE questions, ${HS_N_LAYERS} layers, ${HS_N_GPUS} GPUs (internal sharding), taxonomy=$HS_TAXONOMY"
fi
