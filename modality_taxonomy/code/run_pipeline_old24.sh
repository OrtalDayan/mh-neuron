...
# FT label fallback: use full dir only for labels (not activations)
...
if [[ -z "$_ft_has_labels" ]]; then
    ...
    if [[ -n "$_full_ft_has_labels" ]]; then
        echo "  [viz] No FT label files in $VIZ_DATA_DIR — falling back to full results for labels only"
        VIZ_FT_DIR="$_VIZ_FT_FULL"
        # Note: VIZ_DATA_DIR stays as current mode (for activations/npz files)
    fi
fi
# PMBT labels always come from full dir — test mode never runs permutation classification
VIZ_PMBT_DIR="results/3-classify/full/$MODEL_NAME/llm_permutation"
echo "  [viz] PMBT labels: $VIZ_PMBT_DIR"
