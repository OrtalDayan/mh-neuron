...
def create_neuron_panel(img_array, vis_acts, txt_acts, txt_len,
    ...
    # Line 2: FT classification + pv/pt/pm/pu probabilities
    line2 = (f'FT:   {ft_label_display}'
             f'  (pv={pv:.4f}, pt={pt:.4f}, pm={pm:.4f}, pu={pu:.4f})')

    # Line 3: PMBT classification + p_vis / p_txt statistical p-values
    # Uses p_vis / p_txt (not pv/pt) to distinguish statistical test values
    # from FT classification probabilities.
    if pmbt_label is not None:
        ...
        def _fmt(v):
            return f'{v:.4e}'                                # always scientific notation for exact p-values
        # PMBT displays p_value and D (observed_rate_diff = vis_rate - txt_rate)
        pmbt_parts = [f'p_value={_fmt(p_vis)}']                        # p_vis holds p_value
        if pmbt_p_pref is not None:
            pmbt_parts.append(f'D={pmbt_p_pref:+.4f}')                # D = rate_vis - rate_txt
...
def main():
    ...
    if args.fig3:
        ...
        show_pmbt = getattr(args, 'viz_taxonomy', 'both') in ('pmbt', 'both')
        has_pmbt = (show_pmbt
                    and args.pmbt_data_dir is not None
                    and os.path.isdir(args.pmbt_data_dir))
        if has_pmbt:
            print(f'  PMBT data dir: {args.pmbt_data_dir}')
        else:
            print(f'  WARNING: PMBT disabled — '
                  f'viz_taxonomy={getattr(args, "viz_taxonomy", "both")!r}, '
                  f'pmbt_data_dir={args.pmbt_data_dir!r}, '
                  f'dir_exists={args.pmbt_data_dir is not None and os.path.isdir(args.pmbt_data_dir)}')
                ...
                try:
                    try:
                        ...
                        _pmbt_pre = load_neuron_labels(
                            ...
                            label_file='neuron_labels.json')
                    _pi = _pmbt_pre[nidx]
                    pmbt_label_val    = _pi.get('label', label)
                    # PMBT JSON keys (from classify_neuron_permutation return dict):
                    #   p_value, observed_visual, observed_text, observed_rate_diff
                    pmbt_p_vis_val  = _pi.get('p_value', 1.0)             # p-value, default 1.0 if missing
