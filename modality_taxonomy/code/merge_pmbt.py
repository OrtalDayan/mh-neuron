...
# Newer model classes — InternVL's pinned venv (transformers 4.45.2) lacks
# these; the modern_vlms venv (4.57+) has them. Import optionally so the
# script still runs from either venv; the branches that use these classes
# will only be reachable from the modern venv anyway.
try:
...
def merge_models(model1_path, model2_path, output_dir, alpha, mode='base',
                 ...
                 alpha_unknown=None, alpha_other=None, kv_merge=False,
    ...
    def _layer_in_range(layer_num):
        ...
        layer_start is inclusive; layer_end is INCLUSIVE (matches pipeline's convention)."""
    ...
    # ── Load models ──
    # Branch ordering is order-sensitive: more-specific path patterns must
    # come before less-specific ones (e.g. 'llava-1.5' before 'llava';
    # 'Qwen2.5-VL' before 'Qwen').
    print(f'\n  Loading VLM: {model1_path}')
    _lm_head = None

    def _capture_lm_head(full_model):
        """Save lm_head separately so it doesn't get blended (we exclude it)."""
        if hasattr(full_model.language_model, 'lm_head'):
            print(f'  [lm_head] Found lm_head in language_model')
            return full_model.language_model.lm_head.weight.data.clone()
        if hasattr(full_model, 'lm_head'):
            print(f'  [lm_head] Found lm_head at top level')
            return full_model.lm_head.weight.data.clone()
        print(f'  [warn] No lm_head — falling back to embed_tokens (tied weights)')
        return full_model.language_model.get_input_embeddings().weight.data.clone()

    if 'llava-1.5' in model1_path or 'llava-v1.5' in model1_path:
        # LLaVA-1.5 — older LlavaForConditionalGeneration class.
        _full_model = LlavaForConditionalGeneration.from_pretrained(
            model1_path, dtype=torch.float16,
            low_cpu_mem_usage=True, attn_implementation="eager",
            trust_remote_code=True,
        )
        _lm_head = _capture_lm_head(_full_model)
    ...
    elif 'onevision' in model1_path.lower():
        # LLaVA-OneVision — Qwen2-7B language model wrapped in
        # LlavaOnevisionForConditionalGeneration. Donor: Qwen2-Math-7B.
        if LlavaOnevisionForConditionalGeneration is None:
            raise ImportError(
                'LlavaOnevisionForConditionalGeneration not available in this transformers '
                'version. Run merge_pmbt.py from modern_vlms/.venv (transformers >= 4.45).'
            )
        _full_model = LlavaOnevisionForConditionalGeneration.from_pretrained(
            model1_path, dtype=torch.float16,
            low_cpu_mem_usage=True, attn_implementation="eager",
            trust_remote_code=True,
        )
    ...
    elif 'llava' in model1_path:
        # LLaVA-Next (mistral, llama3, etc.) — LlavaNextForConditionalGeneration.
    ...
    elif 'Qwen2.5-VL' in model1_path or 'Qwen2_5-VL' in model1_path or 'Qwen2.5_VL' in model1_path:
        # Qwen2.5-VL — Qwen2_5_VLForConditionalGeneration.
        # Same wrapper structure as Qwen2-VL: use .model directly (NOT
        # .model.language_model) so the saved state_dict keys match the
        # target Qwen2_5_VL.model.* prefix when loaded back for eval.
        if Qwen2_5_VLForConditionalGeneration is None:
            raise ImportError(
                'Qwen2_5_VLForConditionalGeneration not available in this transformers '
                'version. Run merge_pmbt.py from modern_vlms/.venv (transformers >= 4.49).'
            )
        ...
        # Match BRV's merge.py exactly: use .model (Qwen2VLModel) directly, NOT
        # .model.language_model. Newer transformers expose a .language_model
        # sub-module on Qwen2VLModel; extracting it produces a state_dict with
        # bare keys (`layers.0.*`) that won't match the target HF checkpoint
        # (`model.layers.0.*`) when the merged .pth is loaded back for eval —
        # strict=False silently drops every merged tensor.
    ...
    elif 'InternVL' in model1_path or 'internvl' in model1_path.lower():
        # InternVL — custom class loaded via AutoModel + trust_remote_code.
        # The wrapper exposes .language_model (an InternLM2ForCausalLM).
        # InternLM2 uses different state_dict keys than LLaMA:
        #   tok_embeddings.weight (not embed_tokens.weight),
        #   output.weight (not lm_head.weight),
        #   layers.*.attention.wqkv.weight (not self_attn.{q,k,v}_proj),
        #   layers.*.feed_forward.{w1,w2,w3}.weight (not mlp.{gate,down,up}_proj).
        # For uniform-mode merging these naming differences don't matter (we
        # blend element-wise regardless of role); PMBT-mode would need
        # is_mlp_weight/is_attn_weight to be taught the InternLM2 names.
        # InternVL's custom HF-cache modeling code is on the older transformers
        # convention and accepts torch_dtype= but rejects the newer dtype= kwarg
        # (TypeError: InternVLChatModel.__init__() got an unexpected keyword 'dtype').
        # We pass torch_dtype= explicitly here; everywhere else in this file the
        # newer dtype= is fine because those code paths use stock transformers
        # classes (LLaVA, Qwen2VL, etc.).
        ...
        del _full_model
        model2 = AutoModelForCausalLM.from_pretrained(
            model2_path, torch_dtype=torch.float16,
            low_cpu_mem_usage=True, trust_remote_code=True,
        )
        excluded_keys = {'model.tok_embeddings.weight', 'output.weight'}
    ...
    if mode == 'pmbt':
        ...
        # Unknown-category (and any unlabeled) neurons use alpha_unknown; falls
        # back to the global --alpha when not provided.
        _alpha_unknown = alpha_unknown if alpha_unknown is not None else alpha
        ...
        print(f'  Alpha values: text={alpha_text}, visual={alpha_visual}, '
              f'multimodal={alpha_multimodal}, unknown={_alpha_unknown}')
    ...
    if mode in ['ties', 'dareties', 'darelinear']:
        ...
        for key in tqdm(list(state_dict2.keys()), desc='PMBT merge'):
            ...
            if is_mlp_weight(key) and layer_num is not None:
                ...
                alpha_mask = build_alpha_mask_mlp(
                    ...
                    alpha_multimodal, _alpha_unknown,
                ...
                alpha_mask = build_alpha_mask_kv(
                    layer_num, key, state_dict1[key].shape,
                    pmbt_labels_attn, alpha_text, alpha_visual,
                    alpha_multimodal, _alpha_unknown, num_q_heads,
                    signal_mode=signal_mode,
                    scale_C=scale_C, norm_scale=norm_scale)
                alpha_mask = alpha_mask.to(dtype=state_dict1[key].dtype,
                                          device=state_dict1[key].device)
        ...
        if n_skipped_mlp or n_skipped_attn or n_skipped_other:
            print(f'  Skipped (kept VLM baseline): {n_skipped_mlp} MLP, {n_skipped_attn} attention, {n_skipped_other} other')
    ...
    # Restore the LM head we captured pre-merge (it's excluded from blending,
    # so without restoration the saved state_dict would be missing it). The
    # output-key name depends on the language-model family:
    #   LLaVA-1.5 / LLaVA-Next / LLaVA-OneVision → 'lm_head.weight'
    #   InternVL (InternLM2 LM)                  → 'output.weight'
    if _lm_head is not None:
        if 'InternVL' in model1_path or 'internvl' in model1_path.lower():
            state_dict1['output.weight'] = _lm_head
            print(f'  [lm_head] Added output.weight (InternLM2 head) to saved state dict')
        elif 'llava' in model1_path:
...
def main():
    ...
    parser.add_argument('--mode', type=str, default='pmbt',
    ...
    parser.add_argument('--alpha_visual', type=float, default=1.0)
    parser.add_argument('--alpha_multimodal', type=float, default=1.0)
    parser.add_argument('--alpha_unknown', type=float, default=1.0)
    parser.add_argument('--alpha_other', type=float, default=1.0)
    ...
    parser.add_argument('--layer_end', type=int, default=None,
                        help='Last layer idx (INCLUSIVE) for merging. '
                             'Layers above are kept at VLM baseline. None = no restriction.')
    ...
    merge_models(
        ...
        alpha_unknown=args.alpha_unknown,
