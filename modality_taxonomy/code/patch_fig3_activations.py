...
FIG3_NEURONS_BY_MODEL = {
    ...
    'qwen2vl':           [],                                                     # Qwen2.5-VL-7B (loaded from JSON)
    'llava-ov':          [],                                                     # LLaVA-OneVision-7B (loaded from JSON)
}
...
def get_layer_name(model_type, layer_idx):
    ...
    Line-by-line:
        ...
        - Qwen2.5-VL / LLaVA-OV: model.language_model.layers.{i}.mlp.act_fn
    """
    ...
    elif model_type in ('qwen2vl', 'llava-ov'):                            # Qwen2.5-VL / LLaVA-OneVision
        return f'model.language_model.layers.{layer_idx}.mlp.act_fn'
...
def load_model(model_type, model_path, device):
    ...
    try:
        ...
        if model_type == 'llava-hf':
            ...
            from neuron_modality_statistical import load_model_qwen2vl      # Qwen2.5-VL / LLaVA-OV loader
...
def prepare_inputs(model_type, processor, model, img, text, device,
    ...
    try:
        ...
        if model_type == 'llava-hf':
            ...
            from neuron_modality_statistical import prepare_inputs_qwen2vl  # dynamic visual tokens
...
def build_prompt_text(model_type, description):
    ...
    """
    if model_type in ('internvl', 'qwen2vl', 'llava-ov'):
