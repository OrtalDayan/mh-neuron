...
if [[ "$STEERING_TAXONOMY" == "ft" ]]; then
    HS_LABEL_DIR="$LABELS_BASE/$MODEL_NAME/llm_fixed_threshold"
    HS_TAXONOMY="ft"
elif [[ "$STEERING_TAXONOMY" == "pmbt" ]]; then
    HS_LABEL_DIR="$LABELS_BASE/$MODEL_NAME/llm_permutation"
    HS_TAXONOMY="pmbt"
else
    HS_LABEL_DIR="$LABELS_BASE/$MODEL_NAME/llm_permutation"
    HS_TAXONOMY="pmbt"
...
# ── Test mode: 20 POPE questions, 1 GPU ──────────────────────
if [[ "$MODE" == "test" ]]; then
    HS_N_POPE=20
    HS_N_GPUS=1
    echo "  [test] 20 POPE questions, 1 GPU, taxonomy=$HS_TAXONOMY"
else
    HS_N_POPE="${HALLUC_POPE_SAMPLES:-500}"
    HS_N_GPUS=$HALLUC_SCORE_SHARDS
    echo "  [full] ${HS_N_POPE} POPE questions, ${HS_N_GPUS} GPUs (internal sharding), taxonomy=$HS_TAXONOMY"
fi
...
if $LOCAL; then
    ...
    BSUB_ARGS=(-q "$QUEUE" -J "$JOB_NAME" \
        ...
        -eo "$WORK_DIR/$STEP_LOG_DIR/${JOB_NAME}${LOG_SUFFIX}.err")
    if (( HS_N_GPUS > 1 )); then
        BSUB_ARGS+=(-R "rusage[ngpus_physical=$HS_N_GPUS]")
    fi
