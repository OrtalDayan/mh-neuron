...
def load_neuron_labels(data_dir, layer, model_type='llava-hf',
                       label_file='neuron_labels.json', hook_point='gate'):
    ...
    # Fallback: try act_fn path if hook-specific path doesn't exist
    # (backward compat — old runs used act_fn for everything)
    if not os.path.isfile(label_path) and hook_point != 'gate':
        fallback_suffix = 'feed_forward.act_fn' if model_type == 'internvl' else 'mlp.act_fn'
        fallback_path = os.path.join(data_dir, f'{prefix}.{fallback_suffix}', label_file)
...
def get_description_tokens(tokenizer, img_id, descriptions, model_type='llava-hf'):
    ...
    if model_type in ('llava-hf',):                                          # HF tokenizer from AutoProcessor
        prefix = "USER: <image>\nCould you describe the image?\nASSISTANT:"
        full = f"USER: <image>\nCould you describe the image?\nASSISTANT: {desc_text}"
    elif model_type in ('internvl', 'qwen2vl', 'llava-ov', 'llava-llama3', 'idefics2'):
...
def render_text_with_activations(ax, tokens, activations, max_chars_per_line=110):
    ...
    # Layout parameters — balanced for readability and fitting long descriptions
    font_size = 9                                                   # smaller font to fit more lines
    line_height = 0.055                                             # compact rows (~18 visible lines)
    char_width = 0.0095                                             # narrower chars → more per line
...
def create_neuron_panel(img_array, vis_acts, txt_acts, txt_len,
                        ...
                        pmbt_hook_name='PMBT',
                        pmbt_label_2=None,
                        pmbt_p_vis_2=None, pmbt_p_pref_2=None,
                        pmbt_hook_name_2='PMBT gate',
                        viz_taxonomy='both',
                        suppress_line1=False):
    ...
    # ── Build figure ──────────────────────────────────────────
    # Count header lines to size it
    _n_header_lines = 0 if suppress_line1 else 1
    if viz_taxonomy in ('ft', 'both'):
        _n_header_lines += 1
    if viz_taxonomy in ('pmbt', 'both') and pmbt_label is not None:
        _n_header_lines += 1
    if viz_taxonomy in ('pmbt', 'both') and pmbt_label_2 is not None:
        _n_header_lines += 1
    header_ratio = 0.04 + 0.03 * _n_header_lines
    ...
    if show_pmbt and pmbt_label is not None:
        ...
        line3 = f'{pmbt_hook_name.replace("_", " ")}: {pmbt_label_display}  ({", ".join(pmbt_parts)})'
    else:
        line3 = None

    # Line 4: PMBT hook 2 (optional)
    if show_pmbt and pmbt_label_2 is not None:
        pmbt_label_display_2 = _label_map.get(
            pmbt_label_2, pmbt_label_2.capitalize() + ' neuron')
        p_vis_2 = pmbt_p_vis_2 if pmbt_p_vis_2 is not None else 0.0
        pmbt_parts_2 = [f'p={_fmt(p_vis_2)}']
        if pmbt_p_pref_2 is not None:
            pmbt_parts_2.append(f'D={pmbt_p_pref_2:+.4f}')
        line4 = f'{pmbt_hook_name_2.replace("_", " ")}: {pmbt_label_display_2}  ({", ".join(pmbt_parts_2)})'
    else:
        line4 = None

    # Assemble header from whichever lines are enabled
    lines = [] if suppress_line1 else [line1]
    if show_ft:
        lines.append(line2)
    if line3:
        lines.append(line3)
...
def parse_args():
    ...
    p.add_argument('--hook_point', default='gate',
                   ...
                   help='Hook point: gate/gate_up for FFN, attn for attention heads')
    ...
    p.add_argument('--pmbt_data_dir_2', default=None,
                   help='Second PMBT dir (e.g. gate hook) for multi-hook comparison. '
                        'Shows an extra line in Figure 3 headers.')
    p.add_argument('--pmbt_hook_name', default='PMBT gate_up',
                   help='Label for first PMBT hook in figure headers')
    p.add_argument('--pmbt_hook_name_2', default='PMBT',
                   help='Label for second PMBT hook in figure headers')
    p.add_argument('--suppress_line1', action='store_true',
                   help='Suppress panel/layer/neuron line in header '
                        '(for combined hook figures where suptitle is added by combiner)')
...
def main():
    ...
    try:
        ...
        sampled_ids = None
        print('  WARNING: sampled_ids.json not found (fig3 mode does not need it)')
    ...
    _hf_id_defaults = {
        ...
        'qwen25vl-7b': 'modern_vlms/pretrained/Qwen2.5-VL-7B-Instruct',
        'qwen25vl-3b': 'modern_vlms/pretrained/Qwen2.5-VL-3B-Instruct',
        'internvl': 'modern_vlms/pretrained/InternVL2_5-8B',
        'llava-llama3': 'llava-hf/llama3-llava-next-8b-hf',
        'idefics2': 'modern_vlms/pretrained/idefics2-8b',
    }
    # Use original model_type (before normalization) for hf_id lookup
    if args.hf_id == 'llava-hf/llava-1.5-7b-hf' and _original_model_type in _hf_id_defaults:
        args.hf_id = _hf_id_defaults[_original_model_type]
    if args.model_type in ('llava-hf', 'llava-llama3'):
        ...
        from transformers import AutoTokenizer
        tokenizer = AutoTokenizer.from_pretrained(args.hf_id)       # Mistral tokenizer (from Idefics2)
    else:
        from transformers import AutoTokenizer
        tokenizer = AutoTokenizer.from_pretrained(
    ...
    if args.fig3:
        ...
        for entry in fig3_neurons:
            ...
            try:
                ...
                if sampled_ids is None:
                    raise FileNotFoundError('sampled_ids not loaded')
                ...
                # Test / patch-only mode: no heap — patch writes (e)→rank 0, (f)→rank 1
                _used = used_rank_by_neuron.get((layer, nidx), set())
                rank_idx = len(_used)                                   # 0 for first entry, 1 for second
                used_rank_by_neuron.setdefault((layer, nidx), set()).add(rank_idx)
                print(f'  [test mode] No heap — using act_pattern rank {rank_idx}')
            ...
            # ── Pre-fetch PMBT hook 2 (optional second hook) ────────
            pmbt_label_val_2 = None
            pmbt_p_vis_val_2 = None
            pmbt_p_pref_val_2 = None
            if args.pmbt_data_dir_2 is not None:
                try:
                    _search_2 = [
                        (args.pmbt_data_dir_2, 'neuron_labels_permutation.json'),
                        (args.pmbt_data_dir_2, 'neuron_labels.json'),
                    ]
                    _pmbt_pre_2 = None
                    for _sdir, _sfn in _search_2:
                        try:
                            _pmbt_pre_2 = load_neuron_labels(
                                _sdir, layer, args.model_type,
                                label_file=_sfn,
                                hook_point='gate')  # second hook is always gate
                            print(f'  PMBT hook 2 loaded from {_sdir}/{_sfn}')
                            break
                        except FileNotFoundError:
                            continue
                    if _pmbt_pre_2 is not None:
                        _pi2 = _pmbt_pre_2[nidx]
                        pmbt_label_val_2 = _pi2.get('label', label)
                        pmbt_p_vis_val_2 = _pi2.get('p_value', 1.0)
                        pmbt_p_pref_val_2 = _pi2.get('observed_rate_diff', 0.0)
                except (FileNotFoundError, IndexError, KeyError, TypeError) as e:
                    print(f'  WARNING: PMBT hook 2 pre-fetch failed: {e}')
            ...
            try:
                ...
                _ft_full_dir = os.path.join(
                    ...
                    args.model_name, f'llm_fixed_threshold{_hook_suffix}')
                try:
                    ft_labels = load_neuron_labels(
            ...
            fig_ft = create_neuron_panel(
                ...
                panel=panel,
                pmbt_label=pmbt_label_val,
                pmbt_p_vis=pmbt_p_vis_val,
                pmbt_p_pref=pmbt_p_pref_val,
                pmbt_hook_name=getattr(args, 'pmbt_hook_name', 'PMBT gate_up'),
                pmbt_label_2=pmbt_label_val_2,
                pmbt_p_vis_2=pmbt_p_vis_val_2,
                pmbt_p_pref_2=pmbt_p_pref_val_2,
                pmbt_hook_name_2=getattr(args, 'pmbt_hook_name_2', 'PMBT gate'),
                viz_taxonomy=getattr(args, 'viz_taxonomy', 'both'),
                suppress_line1=getattr(args, 'suppress_line1', False),
            )
            # Store figure object directly — no disk write per panel
            ft_panel_paths.append(fig_ft)                          # list of Figure objects

            # Save individual panel PNG for hook comparison
            _panel_dir = os.path.join(args.output_dir, 'panels')
            os.makedirs(_panel_dir, exist_ok=True)
            _panel_path = os.path.join(_panel_dir, f'fig3_panel_{panel}_{args.model_name}.png')
            fig_ft.savefig(_panel_path, dpi=args.dpi, bbox_inches='tight', facecolor='white')
            print(f'  Saved panel: {_panel_path}')

            # Save panel metadata for combiner suptitles
            import json as _json
            _meta_path = _panel_path.replace('.png', '_meta.json')
            with open(_meta_path, 'w') as _mf:
                _json.dump({'panel': panel, 'layer': layer, 'neuron_idx': nidx,
                            'label': label, 'model_name': args.model_name}, _mf)
        ...
        def _make_combined_grid(figs, entries, suffix, title_extra=''):
            ...
            fig_c, axes_c = plt.subplots(2, 3, figsize=(36, 28))
        ...
        if False:  # PMBT-only grid disabled — info already in FT panel titles
            ...
            if len(ft_panel_paths) == 6 and len(pmbt_panel_paths) == 6:
                ...
                fig_cmp, axes_cmp = plt.subplots(
                    2, 6, figsize=(60, 28))                            # 2 rows × 6 cols
