...
def parse_args():
    ...
    p.add_argument('--model_type', default='llava-liuhaotian',              # Line 2: which model backend to use
                   choices=['llava-hf', 'llava-liuhaotian', 'internvl',
                            'qwen2vl', 'llava-ov', 'idefics2', 'llava-llama3'],
...
def ablation_worker(gpu_id, layer_range, args, return_dict):
    ...
    for layer_idx in range(layer_range[0], layer_range[1]):                # Line 22: iterate layers assigned to this GPU
        ...
        # Per-layer checkpoint: save scores so a walltime timeout doesn't lose this layer.
        try:
            _ckpt_path = os.path.join(
                args.output_dir,
                f'ablation_scores_L{layer_idx}_b{args.batch_neurons}.json')
            _ckpt_data = {
                'layer_idx': layer_idx,
                'batch_neurons': args.batch_neurons,
                'n_neurons': args.n_neurons,
                'neuron_scores': {f'{k[0]}_{k[1]}': v
                                   for k, v in neuron_scores.items()
                                   if k[0] == layer_idx},
                'neuron_scores_tqa': {f'{k[0]}_{k[1]}': v
                                       for k, v in neuron_scores_tqa.items()
                                       if k[0] == layer_idx},
                'baseline_hr': baseline_hr,
                'baseline_tqa_er': baseline_tqa_er if 'baseline_tqa_er' in dir() else None,
            }
            os.makedirs(args.output_dir, exist_ok=True)
            with open(_ckpt_path, 'w') as _f:
                json.dump(_ckpt_data, _f, indent=2)
            print(f'[GPU {gpu_id}] Checkpoint saved: {_ckpt_path} '
                  f'({sum(1 for k in neuron_scores if k[0]==layer_idx)} neurons)')
        except Exception as _e:
            print(f'[GPU {gpu_id}] WARNING: checkpoint save failed: {_e}')
...
def main():
    ...
    if args.skip_ablation:                                                 # Line 6: use pre-computed scores
        ...
        _bsuffix = f'_b{args.batch_neurons}'; scores_path = os.path.join(args.output_dir, f'ablation_scores{_bsuffix}.json')  # Line 11: output path
        ...
        if baseline:                                                       # Line 14: save baseline evaluation
            baseline_path = os.path.join(args.output_dir, f'baseline_results_b{args.batch_neurons}.json')
            with open(baseline_path, 'w') as f:
                json.dump(baseline, f, indent=2)
    ...
    if neuron_scores_tqa:                                                  # Line 21a: only if TQA ablation was run
        tqa_scores_path = os.path.join(args.output_dir, f'ablation_scores_tqa_b{args.batch_neurons}.json')
