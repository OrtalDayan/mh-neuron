...
#
# Chains:  1 describe → 2 merge_desc → 3 classify → 4 merge_class → 5 ablation → 6 visualize → 7 attention → 8 statistics
...
ABLATION_TAXONOMY="both"       # ft | pmbt | both — which taxonomy to ablate (used when --ablation-all is NOT set)
...
case "$STEP" in
    ...
    *) echo "ERROR: unknown step '$STEP'"; echo "  Valid: 1-8, find_fig3, check_collisions, all, all_att"; exit 1 ;;
esac
...
echo "  Ablation taxonomy:  $ABLATION_TAXONOMY (ft | pmbt | both)"
...
if [[ "$ABLATION_TAXONOMY" == "pmbt" || "$ABLATION_TAXONOMY" == "both" ]]; then
    ...
    submit_ablation_job "perm" "perm" "$EXTRA_PRUNE" "$LABELS_DIR" "perm"
fi
...
if [[ "$MODE" == "test" ]]; then
    ...
    OUT_SUFFIX="_test_${MODEL_TYPE}"
    SHARDS_EFFECTIVE=1   # no sharding in test mode
    $QUEUE_SET || QUEUE="waic-short"  # use short queue for quick tests
else
    # Full run: sharded across GPUs
    GEN_ARGS=""
    _DS="_${MODEL_NAME}${DESC_SUFFIX_USER:+_${DESC_SUFFIX_USER}}"
...
submit_ablation_job() {
    ...
    rm -f "$WORK_DIR/$STEP_LOG_DIR/${MERGE_NAME}${LOG_SUFFIX}.log" \
          "$WORK_DIR/$STEP_LOG_DIR/${MERGE_NAME}${LOG_SUFFIX}.err"
    bsub -q short -J "$MERGE_NAME" -w "$SHARD_JOB_NAMES" \
        -oo "$WORK_DIR/$STEP_LOG_DIR/${MERGE_NAME}${LOG_SUFFIX}.log" \
        -eo "$WORK_DIR/$STEP_LOG_DIR/${MERGE_NAME}${LOG_SUFFIX}.err" \
        "cd $WORK_DIR && $PYTHON $ABLATION_SCRIPT \
            --merge_shards $JOB_OUT/shards \
...
LABELS_DIR="$LABELS_BASE/$MODEL_NAME/llm_fixed_threshold"
submit_ablation_job "ft" "" "$EXTRA_PRUNE" "$LABELS_DIR" "xu"
...
# # if $LOCAL; then
# #     PYTHON="$PYTHON" PLOT_SCRIPT="$PLOT_SCRIPT" MODEL_TYPE="$MODEL_TYPE" MODEL_NAME="$MODEL_NAME" \
# #     PLOT_FT_DIR="$PLOT_FT_DIR" PLOT_PMBT_DIR="$PLOT_PMBT_DIR" \
# #     PLOT_OUT_BASE="$PLOT_OUT_BASE" PLOT_DPI="$PLOT_DPI" WORK_DIR="$WORK_DIR" \
# #         bash "$PLOT_WRAPPER" 2>&1 | tee "${STEP_LOG_DIR}/${JOB_NAME}${LOG_SUFFIX}.log"
# #     if [[ ${PIPESTATUS[0]} -eq 0 ]]; then
# #         touch "$PLOT_MARKER"
...
# # ── Validate model-type ─────────────────────────────────────────────────────
# _VALID_MODELS="llava-liuhaotian llava-hf llava-ov internvl qwen2vl all"
# _VALID_ALIASES="llava intern qwen"
# if [[ "$MODEL_TYPE" != *","* ]]; then
#     _found=false
#     for _vm in $_VALID_MODELS; do
...
# # --model-type all              → run for all 4 backends sequentially
# # --model-type internvl,qwen2vl → run for selected backends
# ALL_MODELS=(llava-liuhaotian llava-hf llava-ov internvl qwen2vl)
# if [[ "$MODEL_TYPE" == "all" ]] || [[ "$MODEL_TYPE" == *","* ]]; then
#     if [[ "$MODEL_TYPE" == "all" ]]; then
#         RUN_MODELS=("${ALL_MODELS[@]}")
#     else
...
# # not 32.  Override the default so sharding in step 3 does not produce
# # out-of-range layer indices.  InternVL2.5-8B uses InternLM2 (32 layers).
# if [[ "$MODEL_TYPE" == "qwen2vl" || "$MODEL_TYPE" == "llava-ov" ]]; then
#     N_LAYERS=28
# fi
...
#         exit 1
#     fi
# elif [[ "$MODEL_TYPE" == "qwen2vl" || "$MODEL_TYPE" == "llava-ov" ]]; then
#     PYTHON="$WORK_DIR/modern_vlms/.venv/bin/python"
#     if [[ ! -x "$PYTHON" ]]; then
#         echo "ERROR: $PYTHON not found — run:  cd modern_vlms && python -m venv .venv && pip install -r requirements.txt"
#         exit 1
...
# echo ""
# echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
# echo "  STEP 8: All statistical charts (FT + PMBT)"
# echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
# STEP_LOG_DIR="${LOG_DIR}/8-statistics"
# mkdir -p "$STEP_LOG_DIR" "$WORK_DIR/$STEP_LOG_DIR"

# PLOT_FT_DIR="$OUTPUT_DIR/$MODEL_NAME/llm_fixed_threshold"
# PLOT_PMBT_DIR="$OUTPUT_DIR/$MODEL_NAME/llm_permutation"
# PLOT_OUT_BASE="results/8-statistics/$MODEL_NAME/${MODE_DIR}"
# PLOT_DPI=200
# PLOT_MARKER="$PLOT_OUT_BASE/done.marker"
...
# [[ -d "$PLOT_FT_DIR" ]] && run_plot "[1] FT PNG" \
#     --data_dir "$PLOT_FT_DIR" --model_type "$MODEL_TYPE" \
#     --output_dir "$PLOT_OUT_BASE/fixed-threshold" \
#     --title_prefix "$MODEL_NAME Fixed-Threshold" \
#     --dpi "$PLOT_DPI" --format png --fig5 --fig6 --fig7 --line

# [[ -d "$PLOT_FT_DIR" ]] && run_plot "[2] FT fig6 90%" \
#     --data_dir "$PLOT_FT_DIR" --model_type "$MODEL_TYPE" \
#     --output_dir "$PLOT_OUT_BASE/fixed-threshold" \
#     --title_prefix "$MODEL_NAME Fixed-Threshold" \
#     --dpi "$PLOT_DPI" --format png --fig6 --threshold 0.9

# [[ -d "$PLOT_PMBT_DIR" ]] && run_plot "[3] PMBT PNG" \
#     --data_dir "$PLOT_PMBT_DIR" --model_type "$MODEL_TYPE" \
#     --label_file neuron_labels_permutation.json \
#     --output_dir "$PLOT_OUT_BASE/pmbt" \
#     --title_prefix "$MODEL_NAME PMBT" \
#     --dpi "$PLOT_DPI" --format png --fig5 --fig7 --line

# [[ -d "$PLOT_FT_DIR" && -d "$PLOT_PMBT_DIR" ]] && run_plot "[4] Comparison PNG" \
#     --data_dir "$PLOT_FT_DIR" --perm_dir "$PLOT_PMBT_DIR" \
#     --model_type "$MODEL_TYPE" \
#     --output_dir "$PLOT_OUT_BASE/comparison" \
#     --title_prefix "$MODEL_NAME" \
#     --dpi "$PLOT_DPI" --format png --compare

# [[ -d "$PLOT_FT_DIR" ]] && run_plot "[5] FT PDF" \
#     --data_dir "$PLOT_FT_DIR" --model_type "$MODEL_TYPE" \
#     --output_dir "$PLOT_OUT_BASE/pdf" \
#     --title_prefix "$MODEL_NAME Fixed-Threshold" \
#     --dpi 300 --format pdf --fig5 --fig6 --fig7 --line

# [[ -d "$PLOT_FT_DIR" && -d "$PLOT_PMBT_DIR" ]] && run_plot "[6] Comparison PDF" \
#     --data_dir "$PLOT_FT_DIR" --perm_dir "$PLOT_PMBT_DIR" \
#     --model_type "$MODEL_TYPE" \
#     --output_dir "$PLOT_OUT_BASE/pdf" \
#     --title_prefix "$MODEL_NAME" --dpi 300 --format pdf --compare

# [[ -d "$PLOT_PMBT_DIR" ]] && run_plot "[6b] PMBT PDF" \
#     --data_dir "$PLOT_PMBT_DIR" --model_type "$MODEL_TYPE" \
#     --label_file neuron_labels_permutation.json \
#     --output_dir "$PLOT_OUT_BASE/pdf-pmbt" \
#     --title_prefix "$MODEL_NAME PMBT" \
#     --dpi 300 --format pdf --fig5 --fig7 --line

# [[ -d "$PLOT_FT_DIR" ]] && run_plot "[7] FT SVG" \
#     --data_dir "$PLOT_FT_DIR" --model_type "$MODEL_TYPE" \
#     --output_dir "$PLOT_OUT_BASE/svg" \
#     --title_prefix "$MODEL_NAME Fixed-Threshold" \
#     --format svg --fig5 --fig6 --fig7

# [[ -d "$PLOT_PMBT_DIR" ]] && run_plot "[8] PMBT SVG" \
#     --data_dir "$PLOT_PMBT_DIR" --model_type "$MODEL_TYPE" \
#     --label_file neuron_labels_permutation.json \
#     --output_dir "$PLOT_OUT_BASE/svg-pmbt" \
#     --title_prefix "$MODEL_NAME PMBT" \
#     --format svg --fig5 --fig7
...
# # # ── Short model alias for job names ─────────────────────────────────────────
# # case "$MODEL_TYPE" in
# #     llava-liuhaotian) SHORT_MODEL="lav" ;;
# #     llava-hf)         SHORT_MODEL="lavhf" ;;
# #     llava-ov)         SHORT_MODEL="lavov" ;;
# #     internvl)         SHORT_MODEL="int" ;;
# #     qwen2vl)          SHORT_MODEL="q" ;;
# #     *)                SHORT_MODEL="${MODEL_TYPE:0:4}" ;;
# # esac
...
# # # CRITICAL: We must also unset VIRTUAL_ENV and strip PYTHONPATH so the old
# # # .venv site-packages don't leak into the new interpreter via env inheritance.
# # if [[ "$MODEL_TYPE" == "internvl" || "$MODEL_TYPE" == "qwen2vl" || "$MODEL_TYPE" == "llava-ov" ]]; then
# #     PYTHON="$WORK_DIR/modern_vlms/.venv/bin/python"
# #     if [[ ! -x "$PYTHON" ]]; then
# #         echo "ERROR: $PYTHON not found — run:  cd modern_vlms && python -m venv .venv && pip install -r requirements.txt"
# #         exit 1
...
# #     # Quick test: 6 fig3 images, 2 layers, top_n=5, no sharding
# #     GEN_ARGS="--test_fig3"
# #     DESC_FILE="results/1-describe/generated_descriptions_fig3_${MODEL_TYPE}.json"
# #     CLASSIFY_ARGS="--num_images 6 --top_n 5 --layer_start 0 --layer_end 2 --n_permutations 100"
# #     OUT_SUFFIX="_test_${MODEL_TYPE}"
# #     SHARDS_EFFECTIVE=1   # no sharding in test mode
# #     $QUEUE_SET || QUEUE="waic-short"  # use short queue for quick tests
...
# #     DESC_FILE="results/1-describe/generated_descriptions${_DS}.json"
# #     CLASSIFY_ARGS=""
# #     OUT_SUFFIX="${OUT_SUFFIX_USER}"
# #     SHARDS_EFFECTIVE=$SHARDS
# # fi
...
# # # ── --clean guard: only valid in test mode ────────────────────
# # if $CLEAN && [[ "$MODE" != "test" ]]; then
# #     echo "ERROR: --clean is only valid with --mode test (protects full-run results)."
# #     echo "       Re-run with:  bash run_pipeline.sh --mode test --clean ..."
# #     exit 1
# # fi
...
# # # Create directories (no log cleanup — needed for completion detection)
# # mkdir -p "$WORK_DIR/results/1-describe/shards"
# # mkdir -p "$WORK_DIR/$LOG_DIR"
...
# #     echo ""
# #     if $WIPE; then
# #         echo "  ── WIPE MODE (test): deleting results + logs from step $CLEAN_FROM onwards for $MODEL_NAME ──"
# #     else
# #         echo "  ── CLEAN MODE (test): removing markers from step $CLEAN_FROM onwards for $MODEL_NAME ──"
# #     fi
...
# #             [[ -f "$DESC_FILE" ]] && rm -f "$DESC_FILE" && echo "    rm $DESC_FILE"
# #             # Also remove shards
# #             for sf in results/1-describe/shards/gen_desc*"${OUT_SUFFIX}"*.json; do
# #                 [[ -f "$sf" ]] && rm -f "$sf" && echo "    rm $sf"
# #             done
# #         else
# #             [[ -f "$DESC_FILE" ]] && rm -f "$DESC_FILE" && echo "    rm $DESC_FILE (marker)"
...
# #             --original_model_path "$MODEL_PATH" \
# #             --model_path "$MODEL_PATH" \
# #             --output_path "results/1-describe/generated_descriptions.json" \
# #             $GEN_ARGS) \
# #             2>&1 | tee "${STEP_LOG_DIR}/${JOB_NAME}${LOG_SUFFIX}.log"
# #     else
# #         bsub_tiered -q $QUEUE \
...
# #         END_IDX=$(((s + 1) * N_TOTAL_IMAGES / SHARDS_EFFECTIVE))
# #         JOB_NAME="gen_${s}"
# #         SHARD_FILE="results/1-describe/shards/gen_desc${_DS}_shard${s}.json"
...
# # import json, glob, os
# # merged = {}
# # for f in sorted(glob.glob('results/1-describe/shards/gen_desc${_DS}_shard*.json')):
# #     with open(f) as fp:
# #         merged.update(json.load(fp))
# # os.makedirs(os.path.dirname('${DESC_FILE}'), exist_ok=True)
# # with open('${DESC_FILE}', 'w') as fp:
# #     json.dump(merged, fp, indent=2)
# # n_shards = len(glob.glob('results/1-describe/shards/gen_desc${_DS}_shard*.json'))
# # print(f'Merged {len(merged)} descriptions from {n_shards} shards → ${DESC_FILE}')
# # \""
