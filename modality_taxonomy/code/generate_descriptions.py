...
Supports five backends:
  ...
  --model_type qwen2vl    : Qwen2-VL (requires transformers>=4.49, qwen-vl-utils)
  --model_type qwen25vl-7b   : Qwen2.5-VL-7B (requires transformers>=4.49, qwen-vl-utils)
  --model_type qwen25vl-3b: Qwen2.5-VL-3B (requires transformers>=4.49, qwen-vl-utils)
  --model_type idefics2   : Idefics2-8B (requires transformers>=4.49)
...
def parse_args():
    ...
    p.add_argument('--model_type', default='llava-hf',
                   choices=['llava-hf', 'llava-llama3', 'llava-mistral',
                            'llava-liuhaotian', 'internvl', 'qwen2vl', 'qwen25vl-7b', 'qwen25vl-3b', 'idefics2', 'llava-ov'],
                   help='"llava-hf" | "llava-llama3" | "llava-mistral" | "llava-liuhaotian" | "internvl" | "qwen2vl" | "qwen25vl-7b" | "qwen25vl-3b" | "idefics2" | "llava-ov"')   # which backend to use
...
def load_model_llava_next(model_path, device):
    """Load a LLaVA-Next model (Mistral or LLaMA3 backbone).

    Uses AutoModelForVision2Seq which handles LlavaNextForConditionalGeneration
    automatically. Works for both llava-v1.6-mistral-7b-hf and llama3-llava-next-8b-hf.
...
def build_prompt_llava_next(processor, model_type):
    """Build generation prompt for LLaVA-Next models using the chat template.

    LLaVA-Next-Mistral uses [INST] format, LLaVA-Next-LLaMA3 uses <|begin_of_text|> format.
    The processor's apply_chat_template handles both automatically.
    ...
    try:
        ...
        # Fallback for older transformers that don't support chat templates
        prompt = "USER: <image>\nCould you describe the image?\nASSISTANT:"
    return prompt


def generate_description_llava_next(model, processor, prompt, img, device, max_new_tokens, min_new_tokens):
    """Generate a text description using a LLaVA-Next model.

    Same approach as HF LLaVA but uses AutoModelForVision2Seq.

    Returns dict with token_ids, tokens, text.
    """
    inputs = processor(
...
def load_model_qwen2vl(model_path, device):
    """Load Qwen2-VL or Qwen2.5-VL from a local directory or HF Hub.

    Auto-detects which model class to use based on model_path:
      - Paths containing 'Qwen2.5' → Qwen2_5_VLForConditionalGeneration
      - All other Qwen paths         → Qwen2VLForConditionalGeneration
    This enables both the BRV paper's Qwen2-VL-7B-Instruct and the newer
    Qwen2.5-VL-7B-Instruct to be loaded through the same function.

    Returns: (model, processor)
        - model     : Qwen2_5_VLForConditionalGeneration or Qwen2VLForConditionalGeneration
        - processor : AutoProcessor (text + dynamic-resolution image preprocessing)
    """
    from transformers import AutoProcessor                                   # common processor for both model versions

    is_qwen25 = 'Qwen2.5' in model_path or 'qwen2.5' in model_path.lower()  # check if model_path indicates Qwen2.5-VL

    if is_qwen25:                                                            # Qwen2.5-VL branch (newer architecture)
        from transformers import Qwen2_5_VLForConditionalGeneration          # import Qwen2.5-VL specific class
        model_cls = Qwen2_5_VLForConditionalGeneration                       # assign to generic variable for shared loading code
    else:                                                                    # Qwen2-VL branch (BRV paper model)
        from transformers import Qwen2VLForConditionalGeneration             # import Qwen2-VL specific class
        model_cls = Qwen2VLForConditionalGeneration                          # assign to generic variable for shared loading code

    processor = AutoProcessor.from_pretrained(model_path)                    # loads tokenizer + image processor (works for both)
    model = model_cls.from_pretrained(                                       # load the VL model using the detected class
        model_path,
        torch_dtype=torch.bfloat16,                                          # bfloat16 — Qwen's native dtype for both versions
    ).to(device).eval()                                                      # move to GPU, disable dropout
...
def load_model_idefics2(model_path, device):
    ...
    Returns: (model, processor)
        - model     : Idefics2ForConditionalGeneration (vision + Mistral LLM)
    ...
    model = AutoModelForVision2Seq.from_pretrained(                          # load Idefics2 model
        model_path,
        torch_dtype=torch.float16,                                           # float16 — Idefics2's standard inference dtype
...
def generate_description_idefics2(model, processor, img, device, max_new_tokens, min_new_tokens):
    """Generate a text description for a single image using Idefics2.

    Idefics2 uses processor.apply_chat_template() to format the messages
    list into tokenized inputs with image tokens already expanded.

    Steps:
      1. Build a messages list in the Idefics2 chat format (role/content dicts)
      2. Call processor.apply_chat_template() with tokenize=True to get model inputs
      3. Call model.generate(), strip prompt tokens, decode
    ...
    Returns dict with three keys:
        ...
        text      : str        — human-readable description
    """
    messages = [{                                                              # Idefics2 chat format
        "role": "user",
        "content": [
            {"type": "image"},                                                 # image placeholder (PIL passed separately)
            {"type": "text", "text": "Could you describe the image?"},         # question text
        ],
    }]

    # Step 1: format messages → text string with special tokens
    prompt_text = processor.apply_chat_template(                               # converts messages list to formatted string
        messages, add_generation_prompt=True)                                  # append assistant prefix for generation

    # Step 2: tokenize text + preprocess image together
    ...
    # Strip EOS if present
    tokenizer = processor.tokenizer                                            # access underlying Mistral tokenizer
    if len(generated_ids) > 0 and generated_ids[-1].item() == tokenizer.eos_token_id:
        generated_ids = generated_ids[:-1]                                     # remove EOS token
...
def run_test_fig3(args):
    ...
    # Save original model_type for output filename before normalizing
    _original_model_type = args.model_type

    # Normalize qwen25vl-7b / qwen25vl-3b → qwen2vl: all use the same load/generate code,
    # and load_model_qwen2vl auto-detects Qwen2-VL vs Qwen2.5-VL from model_path.
    if args.model_type in ('qwen25vl-7b', 'qwen25vl-3b'):                       # user passed a Qwen2.5-VL variant
        args.model_type = 'qwen2vl'                                          # reuse qwen2vl code path
    ...
    if args.model_type == 'llava-hf':
        ...
        print(f'Loading Idefics2 model: {args.model_path} ...')
        model, processor = load_model_idefics2(args.model_path, device)  # returns model + processor
    ...
    elif args.model_type in ('llava-llama3', 'llava-mistral'):
        _path = args.model_path or args.hf_id
        print(f'Loading LLaVA-Next model: {_path} ...')
        model, processor = load_model_llava_next(_path, device)
        prompt = build_prompt_llava_next(processor, args.model_type)
    ...
    for img_id, label in FIG3_IMAGES.items():                             # iterate over the 6 Figure 3 images
        ...
        if args.model_type == 'llava-hf':
            ...
            result = generate_description_idefics2(                       # Idefics2 generation path
                model, processor, img, device,
                args.max_new_tokens, args.min_new_tokens)
        elif args.model_type == 'llava-ov':
            ...
            result = generate_description_llava_next(
                model, processor, prompt, img, device,
                args.max_new_tokens, args.min_new_tokens)
    ...
    # ── Save results ─────────────────────────────────────────
    output_path = args.output_path.replace('.json', f'_fig3_{_original_model_type}.json')  # append original backend name to filename
    ...
    if args.model_type == 'llava-hf':
        ...
        del model, processor                                              # delete Idefics2 model references
    elif args.model_type == 'llava-ov':
        del model, processor                                              # delete LLaVA-OneVision model references
    elif args.model_type in ('llava-llama3', 'llava-mistral'):
        del model, processor                                              # delete LLaVA-Next model references
...
def main():
    ...
    _min_tokens_by_model = {                                               # only override if user didn't change the default
        ...
        'qwen25vl-7b': 300,                                                # same Qwen2.5 architecture
        'qwen25vl-3b': 300,                                                # same Qwen2.5 architecture
    }
    ...
    # Normalize qwen25vl-7b / qwen25vl-3b → qwen2vl: all use the same load/generate code,
    # and load_model_qwen2vl auto-detects Qwen2-VL vs Qwen2.5-VL from model_path.
    # Done AFTER test_fig3 so run_test_fig3 sees the original model_type for filenames.
    if args.model_type in ('qwen25vl-7b', 'qwen25vl-3b'):                       # user passed a Qwen2.5-VL variant
        args.model_type = 'qwen2vl'                                          # reuse qwen2vl code path
    ...
    if args.model_type == 'llava-hf':
        ...
        print(f'Loading Idefics2 model: {args.model_path} ...')
        model, processor = load_model_idefics2(args.model_path, device)  # Idefics2: model + processor
        tokenizer = None                                                  # embedded in processor
        image_proc = None
    elif args.model_type == 'llava-ov':
        _path = args.model_path or 'modern_vlms/pretrained/llava-onevision-qwen2-7b-ov-hf'
    ...
    for img_id, img_path in tqdm(image_list, desc='Generating'):          # loop over images with progress bar
        ...
        if args.model_type == 'llava-hf':
            ...
            generated_text = generate_description_idefics2(               # Idefics2 generation path
                model, processor, img, device,
                args.max_new_tokens, args.min_new_tokens)
        elif args.model_type == 'llava-ov':
            ...
            generated_text = generate_description_llava_next(
                model, processor, prompt, img, device,
                args.max_new_tokens, args.min_new_tokens)
