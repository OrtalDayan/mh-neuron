...
# Fall back to full-mode FT dir if the current mode has no actual label files.
_VIZ_FT_FULL="results/3-classify/full/$MODEL_NAME/llm_fixed_threshold"
_ft_has_labels=$(find "$VIZ_DATA_DIR" -name "neuron_labels.json" 2>/dev/null | head -1)
if [[ -z "$_ft_has_labels" ]]; then
    _full_ft_has_labels=$(find "$_VIZ_FT_FULL" -name "neuron_labels.json" 2>/dev/null | head -1)
    if [[ -n "$_full_ft_has_labels" ]]; then
        echo "  [viz] No FT label files in $VIZ_DATA_DIR — falling back to full results:"
        echo "        $VIZ_DATA_DIR → $_VIZ_FT_FULL"
        VIZ_DATA_DIR="$_VIZ_FT_FULL"
        VIZ_FT_DIR="$_VIZ_FT_FULL"
    fi
fi
VIZ_PMBT_DIR="$OUTPUT_DIR/$MODEL_NAME/llm_permutation"
# If the PMBT dir for the current mode is empty/missing, fall back to the full-mode results
# so that fig3 headers still show PMBT p-value and D even when running in test mode.
# Fall back to full-mode PMBT dir if the current mode has no actual label files.
# Checks for neuron_labels_permutation.json inside any layer subdir — not just
# whether the directory itself is non-empty (subdirs may exist but be empty).
_VIZ_PMBT_FULL="results/3-classify/full/$MODEL_NAME/llm_permutation"
_pmbt_has_labels=$(find "$VIZ_PMBT_DIR" -name "neuron_labels_permutation.json" -o -name "neuron_labels.json" 2>/dev/null | head -1)
if [[ -z "$_pmbt_has_labels" ]]; then
    _full_has_labels=$(find "$_VIZ_PMBT_FULL" -name "neuron_labels_permutation.json" -o -name "neuron_labels.json" 2>/dev/null | head -1)
    if [[ -n "$_full_has_labels" ]]; then
        echo "  [viz] No PMBT label files in $VIZ_PMBT_DIR — falling back to full results:"
        echo "        $VIZ_PMBT_DIR → $_VIZ_PMBT_FULL"
        VIZ_PMBT_DIR="$_VIZ_PMBT_FULL"
    fi
fi
...
# Always forward --viz-taxonomy so the Python script uses the correct suffix and shows PMBT values
VIZ_ARGS="$VIZ_ARGS --viz-taxonomy $VIZ_TAXONOMY"
...
EXCLUDED_MODELS=("llava-1.5-7b")
for model_dir in "$OUTPUT_DIR"/*/; do
    mname=$(basename "$model_dir")

    # Skip excluded models
    skip=false
    for excl in "${EXCLUDED_MODELS[@]}"; do
        [[ "$mname" == "$excl" ]] && skip=true && break
    done
    $skip && echo "    ✗ $mname (excluded)" && continue
