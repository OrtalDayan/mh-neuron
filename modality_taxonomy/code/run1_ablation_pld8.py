...
def main():
    ...
    if args.label_dir_attn and args.hook_point in ('gate', 'gate_up'):
        ...
        for name, param in model.named_parameters():
            for layer_idx in hmap:
                pattern = f'layers.{layer_idx}.self_attn.o_proj.weight'
                if pattern in name:
    ...
    if args.model_type == 'llava-llama3':
        ...
        lm_state = model.model.state_dict()
    else:
        raise ValueError(f'Unsupported model type: {args.model_type}')
    ...
    if args.model_type == 'llava-llama3':
        # BRV: model.model.language_model.load_state_dict() → LlamaForCausalLM
    ...
    elif args.model_type in ('idefics2', 'qwen2vl'):
        ...
        if sample_key.startswith('model.'):
            tag = 'Idefics2' if args.model_type == 'idefics2' else 'Qwen2VL'
