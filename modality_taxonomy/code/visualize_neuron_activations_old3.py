...
def _norm_coco_id(coco_id):
    """Normalise a COCO image ID to a zero-padded 12-character string.

    sampled_ids.json may store IDs as plain integers (403170), unpadded
    strings ("403170"), or already-padded strings ("000000403170").
    Normalising both sides before comparison guarantees a match regardless
    of how the pipeline serialised the IDs.
    """
    return str(int(str(coco_id))).zfill(12)                         # int() strips leading zeros, zfill re-pads
...
def find_rank_for_image(top_n_sids, neuron_idx, sampled_ids, target_coco_id):
    ...
    """
    target_norm = _norm_coco_id(target_coco_id)                     # normalise target once outside loop
...
def main():
    ...
    if args.fig3:
        ...
        # Track which rank_idx has already been used for each (layer, neuron) pair.
        # This prevents two panels sharing the same neuron (e.g. panels (e) and (f)
        # both use layer 21 / neuron 6100) from being assigned the same rank when
        # their target image falls back.
        used_rank_by_neuron = {}                                        # (layer, nidx) → set of rank_idx
        ...
        for entry in fig3_neurons:
            ...
            if rank_idx is not None:
                ...
                # Image not in this neuron's top-N — fall back to the highest
                # unused rank so panels sharing a neuron (e.g. (e) and (f))
                # are guaranteed to show different images.
                neuron_key = (layer, nidx)                             # key into used-rank tracker
                already_used = used_rank_by_neuron.get(neuron_key, set())
                sorted_ranks = np.argsort(-top_n_acts[nidx])          # descending by activation
                rank_idx = None
                for candidate in sorted_ranks:                         # walk best → worst
                    if int(candidate) not in already_used:             # skip already-assigned ranks
                ...
                print(f'  WARNING: Image {target_coco_id} not in neuron\'s '
                      f'top-N. Falling back to rank {rank_idx} (first unused rank).')

            # Record this rank so the next panel on the same neuron picks a different one
