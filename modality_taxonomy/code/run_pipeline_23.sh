...
if [[ ! -d "$VIZ_PMBT_DIR" || -z "$(ls -A "$VIZ_PMBT_DIR" 2>/dev/null)" ]]; then
    if [[ -d "$_VIZ_PMBT_FULL" && -n "$(ls -A "$_VIZ_PMBT_FULL" 2>/dev/null)" ]]; then
        echo "  [viz] PMBT dir empty/missing for $MODE mode — falling back to full results:"
