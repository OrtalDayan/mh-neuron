...
def plot_panel(ax, records, ranking, bench, cats, title, max_frac,
               show_xlabel=True, show_ylabel=True):
    """One (model, bench) panel: per cat, draw shaded [min,max] band, dashed mean, solid ranked.

    Fractions > max_frac are excluded from this panel (e.g. f=1.0 single-trial spikes).
    """
    # xticks shown — only those ≤ max_frac (avoids dangling labels past the data)
    visible_fracs = [f for f in FRACTIONS_DEFAULT if f <= max_frac]

    for cat in cats:
        rows = aggregate_per_cat(records, ranking, cat, bench)
        if not rows:
            continue
        # Filter rows by max_frac before extracting arrays
        rows = [r for r in rows if r['frac'] <= max_frac]
        if not rows:
            continue
    ...
    ax.set_xscale('log')
    ax.set_xticks(visible_fracs)
    ax.set_xticklabels([f'{f:g}' for f in visible_fracs], fontsize=8)
    # Tighten xlim to the actually-shown fracs (with small padding)
    ax.set_xlim(visible_fracs[0] * 0.8, visible_fracs[-1] * 1.2)
    ax.set_ylim(-2, 102)
...
def plot_figure(args):
    ...
    # Figure-level legend — collect handles from the first axis that has plotted lines.
    # This avoids per-panel legends crowding the data area.
    handles, labels = [], []
    for ax in axes.flat:
        ...
        if h:
            ...
            break
    if handles:
        # ncol=6 fits the 3 cats × 2 line-types in one horizontal strip below the grid
...
def main():
    ...
    p.add_argument('--max-frac', type=float, default=0.5,
                   help='Drop fractions > this from the plot (default 0.5 — excludes the f=1.0 '
                        'single-trial endpoint where outliers commonly appear). '
                        'Pass 1.0 to include all fractions.')
    args = p.parse_args()
    plot_figure(args)
